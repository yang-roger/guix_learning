...
GX_EXTERN_C ULONG            _tx_thread_system_state;
#endif
...
static void CALLBACK gx_win32_timer_expiration(UINT wTimerID, UINT msg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
    ...
    SendMessage(instance->winhandle, WM_TIMER, 0, 0);
}
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    gx_win32_start_multimedia_timer                                     */
/*                                                           6.1.10       */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Ting Zhu, Microsoft Corporation                                     */
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    gx_win32_map_key_to_guix_event                                      */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    gx_win32_get_data_instance_by_win_handle                            */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
int index;
    ...
    for (index = 0; index < GX_MAX_WIN32_DISPLAYS; index++)
    {
        if (win32_instance_data[index].type == 0)
        {
            continue;
        }

        if (win32_instance_data[index].winhandle == winHandle)
        {
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    gx_win32_rotate_canvas_to_bmp_32bpp                                 */
/*                                                           6.1.4        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
INT    write_sign;
    ...
    pReadStart = (ULONG *)canvas->memory;
    pWriteStart = (ULONG *)canvas->padded_memory;
    copy_width = canvas->y_resolution;
    copy_height = canvas->x_resolution;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    gx_win32_rotate_canvas_to_bmp_16bpp                                 */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
INT     write_stride;
    ...
    pReadStart = (USHORT *)canvas->memory;
    pWriteStart = (USHORT *)canvas->padded_memory;
    copy_width = canvas->y_resolution;
    copy_height = canvas->x_resolution;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    gx_win32_rotate_canvas_to_bmp_8bpp                                  */
/*                                                           6.1.4        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
INT       write_stride;
    ...
    pReadStart = (GX_UBYTE *)canvas->memory;
    pWriteStart = (GX_UBYTE *)canvas->padded_memory;
    copy_width = canvas->y_resolution;
    copy_height = canvas->x_resolution;
...
{
VOID       *memptr = (VOID *)canvas->memory;
...
INT         row;
    ...
    if (canvas->padded_memory)
    {
            ...
            (display->rotation_angle == GX_SCREEN_ROTATION_FLIP))
        {
            ...
            read_start = (UCHAR *)canvas->memory;
            source_stride_bytes = canvas->display->driver_row_pitch_get(canvas->x_resolution);
            read_start += dirty->top * source_stride_bytes;

            /* calculate where to copy to */
            write_start = (UCHAR *)canvas->padded_memory;
            dest_stride_bytes = (source_stride_bytes + 3) & 0xfffc;
            write_start += dirty->top * dest_stride_bytes;

            dirty_width = canvas->x_resolution;
            copy_bytes = canvas->display->driver_row_pitch_get(dirty_width);

            /* copy the dirty rows from source to dest */

            for (row = dirty->top; row <= dirty->bottom; row++)
            {
            ...
            memptr = (VOID *)canvas->padded_memory;
        }
        else
        {
            switch (display->color_format)
            {
            case GX_COLOR_FORMAT_24XRGB:
            case GX_COLOR_FORMAT_32ARGB:
                gx_win32_rotate_canvas_to_bmp_32bpp(canvas);
                memptr = canvas->padded_memory;
                break;
...
INT                           ysrc;
    ...
    driver_instance = (GX_WIN32_DISPLAY_DRIVER_DATA *)(canvas->display->driver_data);

    if (driver_instance->ready == 0)
    {
    ...
    gx_utility_rectangle_define(&Limit, 0, 0,
                                canvas->x_resolution - 1,
                                canvas->y_resolution - 1);

    if (gx_utility_rectangle_overlap_detect(&Limit, dirty, &Copy))
    {
        memptr = _win32_canvas_memory_prepare(canvas, &Copy);

        win_device = GetDC(driver_instance->winhandle);
        SetMapMode(win_device, MM_TEXT);

#ifdef GX_ENABLE_CANVAS_PARTIAL_FRAME_BUFFER
        xsrc = Copy.left - canvas->memory_offset_x;
        ysrc = Copy.top - canvas->memory_offset_y;

        driver_instance->bmpinfo.header.biWidth = canvas->memory_width;
        driver_instance->bmpinfo.header.biHeight = canvas->memory_height;
#else
        xsrc = Copy.left;
        ysrc = Copy.top;

        driver_instance->bmpinfo.header.biWidth = canvas->x_resolution;
        driver_instance->bmpinfo.header.biHeight = canvas->y_resolution;
#endif

        gx_utility_rectangle_shift(&Copy, canvas->display_offset_x, canvas->display_offset_y);

        Top = Copy.top;
        Left = Copy.left;
        Width = Copy.right - Copy.left + 1;
        Height = Copy.bottom - Copy.top + 1;

        StretchDIBits(win_device, Left, Top, Width, Height,
                      xsrc,
                      ysrc + Height + 1,
                      Width, -Height, memptr,
                      (BITMAPINFO *)&(driver_instance->bmpinfo),
                      DIB_RGB_COLORS,
                      SRCCOPY);

        ReleaseDC(driver_instance->winhandle, win_device);
    }
}
...
WNDCLASS  wndclass;
    ...
    TargetSize.left = 0;
    TargetSize.top = 0;
    TargetSize.right = gx_driver_ptr->bmpinfo.header.biWidth - 1;
    TargetSize.bottom = gx_driver_ptr->bmpinfo.header.biHeight - 1;
...
GX_BOOL                       check_key_event;
    ...
    NewEvent.sender = 0;
    NewEvent.target = GX_NULL;

    switch (message)
    {
    case WM_TIMER:
        NewEvent.type = GX_EVENT_TIMER;
        NewEvent.payload.ulongdata = 1;
    ...
    case WM_LBUTTONDOWN:
        ...
        NewEvent.type = GX_EVENT_PEN_DOWN;
        NewEvent.payload.pointdata.x = (GX_VALUE)WinX;
        NewEvent.payload.pointdata.y = (GX_VALUE)WinY;
        NewEvent.display_handle = (ULONG)driver_data;
    ...
    case WM_KEYDOWN:
        ...
        if (wParam != VK_PROCESSKEY)
        {
            ...
            {
                event_type = gx_win32_map_key_to_guix_event((USHORT)wParam, shift_flag, ctrl_flag);
            }
            ...
            {
                NewEvent.type = GX_EVENT_KEY_DOWN;
                NewEvent.payload.ushortdata[0] = gx_win32_map_key_to_guix_key((USHORT)wParam);
            }
            gx_win32_event_to_guix(&NewEvent);
        }
        break;
    ...
    case WM_IME_CHAR:
        ...
        NewEvent.payload.ushortdata[0]  = s_ch[0];
        gx_win32_event_to_guix(&NewEvent);
        break;
    ...
    case WM_MOUSEMOVE:
        if (wParam & MK_LBUTTON)
        {
            ...
            NewEvent.type = GX_EVENT_PEN_DRAG;
            NewEvent.payload.pointdata.x = (GX_VALUE)WinX;
            NewEvent.payload.pointdata.y = (GX_VALUE)WinY;
            NewEvent.display_handle = (ULONG)driver_data;
            gx_win32_event_to_guix(&NewEvent);
...
int exit_code = 0;
    ...
    if ((instance->thread_handle != INVALID_HANDLE_VALUE) &&
        instance->thread_handle)
    {
...
GX_WIN32_DISPLAY_DRIVER_DATA *instance = (GX_WIN32_DISPLAY_DRIVER_DATA *)thread_input;
    ...
    instance->winhandle = gx_win32_window_create(instance, gx_win32_event_process, 20, 20);
...
{
    /* Show window. */
    ShowWindow(instance->winhandle, SW_SHOW);
    UpdateWindow(instance->winhandle);
