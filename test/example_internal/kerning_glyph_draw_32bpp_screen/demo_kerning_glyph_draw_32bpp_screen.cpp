...
VOID  demo_thread_entry(ULONG thread_input)
{
    ...
    strcpy(pVSliderInput->buffer, "20");
    pVSliderInput->string_size = 2;

    strcpy(pHSliderInput->buffer, "20");
    pHSliderInput->string_size = 2;
...
INT value;
    ...
    case GX_SIGNAL(ID_VERTICAL_SLIDER, GX_EVENT_SLIDER_VALUE):
        yshift = myevent->payload.longdata;
        yshift = pre_vslider_value - yshift;
        pre_vslider_value = myevent->payload.longdata;
        gx_widget_shift(pHorizontalClip, 0, yshift, GX_TRUE);
        break;

    case GX_SIGNAL(ID_HORIZONTAL_SLIDER, GX_EVENT_SLIDER_VALUE):
        xshift = myevent->payload.longdata;
        xshift -= pre_hslider_value;
        pre_hslider_value = myevent->payload.longdata;
        gx_widget_shift(pVerticalClip, xshift, 0, GX_TRUE);
        break;

    case GX_SIGNAL(ID_VSLIDER_INPUT, GX_EVENT_TEXT_EDITED):
        string_to_int(pVSliderInput->buffer, pVSliderInput->string_size, &value);
        if((value >= 20) && (value <= 345))
        {
            value = 345 - (value - 20);
            gx_slider_value_set(pVerticalSlider, &pVerticalSlider->info, value);
        }
        break;

    case GX_SIGNAL(ID_HSLIDER_INPUT, GX_EVENT_TEXT_EDITED):
        string_to_int(pHSliderInput->buffer, pHSliderInput->string_size, &value);
        if((value >= 20) && (value <= 570))
        {
            gx_slider_value_set(pHorizontalSlider, &pHorizontalSlider->info, value);
        }
        break;
    ...
    default:
        ...
        if(myevent->type == GX_EVENT_PEN_UP)
        {
            gx_widget_front_move(pHorizontalClip, GX_NULL);
            gx_widget_front_move(pVerticalClip, GX_NULL);
        }
        break;
    }
    return status;
...
UINT load_binary_resource_data_to_ram()
{
    ...
    binres_root_address = (GX_UBYTE*)memory_allocate(total_length);
