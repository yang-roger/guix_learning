...
VOID demo_thread_entry(ULONG thread_input)
{
    ...
    gx_studio_named_widget_create("glyph_screen", (GX_WIDGET *) root, GX_NULL);
...
UINT glyph_screen_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_BRUSH_ALPHA_SLIDER, GX_EVENT_SLIDER_VALUE):
        glyph_screen.glyph_screen_view.alpha = event_ptr->payload.ulongdata;
...
GX_STRING text;
    ...
    text.ptr = entry->text;
    text.length = strnlen(entry->text, sizeof(entry->text));
