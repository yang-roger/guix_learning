...
float            x_ratio = 1;
float            y_ratio = 1;
GX_RESOURCE_ID   resize_pixelmap_id = GX_PIXELMAP_ID_ICON_FOOT;
...
VOID update_prompt_text(GX_RESOURCE_ID id, float value)
{
    ...
    if (pp)
    {
        ...
        text[index++] = '0' + (int)(value);
        text[index++] = '.';
        text[index++] = '0' + (int)(value * 10) % 10;
        text[index++] = '0' + (int)(value * 100) % 10;
        text[index] = '\0';
...
UINT resize_screen_event_handler(GX_WINDOW *window, GX_EVENT *myevent)
{
    ...
    case GX_SIGNAL(ID_RADIO_FOOT, GX_EVENT_RADIO_SELECT):
        resize_pixelmap_id = GX_PIXELMAP_ID_ICON_FOOT;
        gx_system_dirty_mark((GX_WIDGET *)&resize_screen.resize_screen_resize_window);
        break;
