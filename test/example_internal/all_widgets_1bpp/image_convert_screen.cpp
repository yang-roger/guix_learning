...
void cleanup_output_pixelmaps()
{
    if (png_to_1bpp_map)
    {
        if (png_to_1bpp_map->data)
        {
            memory_free((VOID *)png_to_1bpp_map->data);
            png_to_1bpp_map->data = GX_NULL;
        }

        if (png_to_1bpp_map->aux_data)
        {
            memory_free((VOID *)png_to_1bpp_map->aux_data);
            png_to_1bpp_map->aux_data = GX_NULL;
        }
        png_to_1bpp_map = GX_NULL;
    }
    ...
    if (jpeg_to_1bpp_map)
    {
        if (jpeg_to_1bpp_map->data)
        {
            memory_free((VOID *)jpeg_to_1bpp_map->data);
            jpeg_to_1bpp_map->data = GX_NULL;
        }

        if (jpeg_to_1bpp_map->aux_data)
        {
            memory_free((VOID *)jpeg_to_1bpp_map->aux_data);
            jpeg_to_1bpp_map->aux_data = GX_NULL;
        }
        jpeg_to_1bpp_map = GX_NULL;
    }
}
