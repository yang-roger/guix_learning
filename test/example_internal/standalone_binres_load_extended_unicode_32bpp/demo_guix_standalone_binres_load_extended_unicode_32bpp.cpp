...
UINT load_standalone_pixelmaps()
{
    ...
    {
        status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmaps, index, &pixelmaps[index]);
...
VOID pixelmap_draw(GX_WINDOW* window)
{
    ...
    {
    case ID_APPLE:
        gx_canvas_pixelmap_draw(size->left, size->top, &pixelmaps[0]);
        break;

    case ID_FISH:
        gx_canvas_pixelmap_draw(size->left, size->top, &pixelmaps[1]);
        break;
    }
