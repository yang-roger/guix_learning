...
void custom_prompt_draw(GX_NUMERIC_PROMPT* prompt)
{
    ...
    if (prompt->style & GX_STYLE_DRAW_SELECTED)
    {
        prompt->font_id_() = GX_FONT_ID_SELECTED;
    }
    else
    {
        prompt->font_id_() = GX_FONT_ID_NORMAL;
    }
...
void widget_scroll_callback(GX_GENERIC_SCROLL_WHEEL* wheel, GX_WIDGET* widget, INT index)
{
    ...
    if (!created)
    {
        ...
        gx_utility_rectangle_define(&size, 0, 0, width - 1, wheel->row_height - 1);
        gx_numeric_prompt_create((GX_NUMERIC_PROMPT*)&row->prompt, "", wheel, 0, GX_STYLE_ENABLED | GX_STYLE_TRANSPARENT, 0, &size);

        size.top = (size.top + size.bottom - SCROLL_WHEEL_ICON_HEIGHT) >> 1;
        size.left = size.right - SCROLL_WHEEL_ICON_WIDTH - 1;

        gx_icon_create(&row->icon, "", &row->prompt, GX_PIXELMAP_ID_I_PATIENTLIST_LG, GX_STYLE_TRANSPARENT, 0, size.left, size.top);
