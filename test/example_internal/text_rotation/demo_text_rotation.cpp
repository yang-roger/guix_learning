...
INT *yshift;
    ...
    if (*xshift || *yshift)
    {
        ...
        if (*xshift > 0)
        {
            if (window->size.right >= MAIN_DISPLAY_X_RESOLUTION)
            {
                *xshift = -*xshift;
            }
        }
        ...
        if (*yshift > 0)
        {
            if (window->size.bottom >= MAIN_DISPLAY_Y_RESOLUTION)
            {
                *yshift = -*yshift;
            }
        }
...
GX_STRING string;
    ...
    string.ptr = string_hello_world;
    string.length = sizeof(string_hello_world) - 1;
    ...
    string.ptr = string_4bpp_font;
    string.length = sizeof(string_4bpp_font) - 1;
    gx_utility_string_to_alphamap_ext(&string, font, &Alphamap_4bit_font);

    gx_widget_font_get(widget, GX_FONT_ID_MONO_FONT, &font);
    string.ptr = string_1bpp_font;
    string.length = sizeof(string_1bpp_font) - 1;
...
void text_window_draw(GX_WINDOW *window)
{
    ...
    if (source_map)
    {
        if(gx_utility_pixelmap_rotate(source_map, text_angle, &rotated_text, &xpos, &ypos) == GX_SUCCESS)
        {
            ...
            xpos -= rotated_text.width / 2;
            ypos -= rotated_text.height / 2;
    ...
    else
    {
        ...
        string.length = strnlen(dynamic_text, sizeof(dynamic_text));
