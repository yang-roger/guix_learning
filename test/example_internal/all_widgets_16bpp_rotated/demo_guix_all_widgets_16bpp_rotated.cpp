...
#include <stdio.h>
#include "gx_api.h"
#include "all_widgets_16bpp_rotated_resources.h"
#include "all_widgets_16bpp_rotated_specifications.h"
...
extern GX_STUDIO_DISPLAY_INFO all_widgets_16bpp_rotated_display_table[];
...
extern UINT win32_graphics_driver_setup_565rgb_rotated(GX_DISPLAY *display);
...
VOID  demo_thread_entry(ULONG thread_input)
{
    ...
    gx_studio_display_configure(MAIN_DISPLAY, win32_graphics_driver_setup_565rgb_rotated,
                                LANGUAGE_ENGLISH, MAIN_DISPLAY_THEME_1, &root);

    /* create the icon screen */
    //gx_studio_named_widget_create("icon_screen", (GX_WIDGET *) root, GX_NULL);
    ...
    string.ptr = "multi line text input";
    string.length = 21;
    gx_multi_line_text_input_text_set_ext(&widget_color_test_screen.widget_color_test_screen_ml_text_input, &string);

    text = "single line input";
    gx_single_line_text_input_character_insert(&widget_color_test_screen.widget_color_test_screen_sl_text_input, (GX_UBYTE *)text, strlen(text));
#endif
    ...
    gx_canvas_create(&composite_canvas, GX_NULL,
        all_widgets_16bpp_rotated_display_table[0].display,
        GX_CANVAS_COMPOSITE,
...
UINT next_button_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(IDB_NEXT, GX_EVENT_CLICKED):
        ...
        case ID_SHAPES_SCREEN:
            ToggleScreen((GX_WINDOW *) &sprite_screen, window);
            break;
