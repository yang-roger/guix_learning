...
extern GX_BOOL day_wheel_selected_row_changed;
extern GX_BOOL month_wheel_selected_row_changed;
...
{
    sprintf(wheel->string_buffer, "%02d", row + 1);
    string->ptr = wheel->string_buffer;
    string->length = 2;
    return GX_SUCCESS;
}
...
VOID select_day()
{
    ...
    if (input && day_wheel)
    {
        selected_day = string_to_int(input->buffer);
...
UINT scroll_wheel_screen_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_DAY_WHEEL, GX_EVENT_LIST_SELECT):
        day_wheel_selected_row_changed = GX_TRUE;
        break;

    case GX_SIGNAL(ID_MONTH_WHEEL, GX_EVENT_LIST_SELECT):
        month_wheel_selected_row_changed = GX_TRUE;
        day_count_update();
        break;
...
VOID day_count_update()
{
    ...
    year = year_wheel->start_val;
    year += year_wheel->selected_row;

    month = month_wheel->selected_row + 1;
    day_count = GX_ABS(day_wheel->end_val - day_wheel->start_val) + 1;
