...
VOID calculate_line_ends(VOID)
{
    ...
    INT x_center = pLineWin->size.left + LINE_CENTER_X_OFFSET;
    INT y_center = pLineWin->size.top + LINE_CENTER_Y_OFFSET;
...
VOID update_width_prompt(VOID)
{
    ...
    if (pp)
    {
        ...
        string.ptr = width_val;
        string.length = strnlen(width_val, sizeof(width_val));
...
VOID update_line_angle_prompt(VOID)
{
    ...
    if (pp)
    {
        ...
        string.ptr = angle_val;
        string.length = strnlen(angle_val, sizeof(angle_val));
        gx_prompt_text_set_ext(pp, &string);
    }
}
...
VOID line_win_draw(GX_WINDOW *window)
{
    ...
    if (brush)
    {
        brush->line_pattern = line_pattern;
