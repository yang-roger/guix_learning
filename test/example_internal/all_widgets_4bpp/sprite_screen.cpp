...
UINT sprite_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_BIRD_SPRITE, GX_EVENT_SPRITE_COMPLETE):
        move_sprite();
        gx_sprite_start(&sprite_screen.sprite_screen_sprite, 0);
        break;
    default:
...
void move_sprite(void)
{
    ...
    if (size.bottom > pSpriteScreen->size.bottom)
    {
        size.top -= pSpriteScreen->size.bottom;
        size.bottom -= pSpriteScreen->size.bottom;
    }
    if (size.top < pSpriteScreen->size.top)
    {
        size.top += pSpriteScreen->size.bottom;
        size.bottom += pSpriteScreen->size.bottom;
    }
    ...
    if (size.right > pSpriteScreen->size.right)
    {
        size.left -= pSpriteScreen->size.right;
        size.right -= pSpriteScreen->size.right;
    }
...
VOID apple_window_draw(GX_WINDOW *window)
{
    ...
    if (gx_utility_pixelmap_resize(map, &scaled_map, width, height) == GX_SUCCESS)
    {
        ...
        gx_canvas_pixelmap_draw(xpos, ypos, &scaled_map);
