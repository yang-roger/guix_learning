...
extern GX_STUDIO_DISPLAY_INFO all_widgets_4bpp_display_table[];
...
extern UINT win32_graphics_driver_setup_4bpp_grayscale(GX_DISPLAY *display);
...
VOID  demo_thread_entry(ULONG thread_input)
{
    ...
    gx_studio_display_configure(MAIN_DISPLAY, win32_graphics_driver_setup_4bpp_grayscale, 
    ...
    /* create the sprite screen */
    gx_studio_named_widget_create("line_screen", (GX_WIDGET *)GX_NULL, (GX_WIDGET **)&pLineScreen);
    ...
    gx_canvas_create(&composite_canvas, GX_NULL,
        all_widgets_4bpp_display_table[0].display,
        GX_CANVAS_COMPOSITE,
    ...
    gx_widget_fill_color_set(&NestedWindow, GX_COLOR_ID_GRAY, GX_COLOR_ID_GRAY, GX_COLOR_ID_GRAY);
...
UINT next_button_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == NUMBER_ANIMATION_TIMER)
        {
            ...
            gx_numeric_pixelmap_prompt_value_set(&text_screen.text_screen_numeric_pixelmap_prompt_1, numeric_prompt_value);
        }
        break;
...
GX_STRING string;
    ...
    string.length = strnlen(string.ptr, sizeof(entry->text));
