...
VOID *load_binary_resource_data_to_ram(GX_CHAR *filename)
{
    ...
    if (p_file)
    {
        ...
        address = (UCHAR*)memory_allocate(total_length);
...
UINT load_standalone_pixelmaps()
{
    ...
    {
        status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmap_565rgb, index, &pixelmap_565rgb[index]);
        ...
        status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmap_8bit_palette, index, &pixelmap_8bit_palette[index]);
    ...
    status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmap_compress_4444argb, 0, &pixelmap_compress_4444argb);
    ...
    status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmap_raw_4444argb, 0, &pixelmap_raw_4444argb);
    ...
    status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmap_compress_alphamap, 0, &pixelmap_compress_alphamap);
    ...
    status = gx_binres_pixelmap_load((GX_UBYTE*)root_address_pixelmap_raw_alphamap, 0, &pixelmap_raw_alphamap);
    ...
    status = gx_binres_font_load((GX_UBYTE*)root_address_font_1bpp, 0, font_1bpp_buffer, &buffer_size);
    ...
    status = gx_binres_font_load((GX_UBYTE*)root_address_font_4bpp, 0, font_4bpp_buffer, &buffer_size);
    ...
    status = gx_binres_font_load((GX_UBYTE*)root_address_font_8bpp, 0, font_8bpp_buffer, &buffer_size);
...
VOID main_screen_draw(GX_WINDOW* window)
{
    GX_VALUE xpos;
    GX_VALUE ypos;
    GX_RECTANGLE* size = &window->size;

    gx_widget_background_draw(window);

    xpos = size->left - (pixelmap_565rgb[0].width);
    ypos = size->top - (pixelmap_565rgb[0].height);
    gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_565rgb[0]);

    xpos = size->right - (pixelmap_565rgb[1].width);
    gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_565rgb[1]);

    xpos = size->left - (pixelmap_565rgb[2].width);
    ypos = size->bottom - (pixelmap_565rgb[2].height);
    gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_565rgb[2]);

    xpos = size->right - (pixelmap_565rgb[3].width);
...
{
    GX_BRUSH* brush;
    GX_RECTANGLE* size = &prompt->size;
    GX_STRING string;

    gx_context_brush_get(&brush);
    gx_context_string_get_ext(prompt->text_id_(), &string);

    switch (prompt->id)
    {
    case ID_TITLE_565RGB:
        brush->font = (GX_FONT*)font_8bpp_buffer;
        break;

    case ID_TITLE_4444ARGB:
        brush->font = (GX_FONT*)font_4bpp_buffer;
        break;

    case ID_TITLE_8BIT_PALETTE:
        brush->font = (GX_FONT*)font_1bpp_buffer;
        break;
...
VOID pixelmap_draw(GX_WINDOW* window)
{
    ...
    case ID_8BIT_PALETTE:
        ...
        xpos = size->right - (pixelmap_8bit_palette[1].width);
        gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_8bit_palette[1]);

        xpos = size->left;
        ypos = size->bottom - (pixelmap_8bit_palette[2].height);
        gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_8bit_palette[2]);

        xpos = size->right - (pixelmap_8bit_palette[3].width);
    ...
    case ID_4444ARGB:
        ...
        ypos = size->top + pixelmap_compress_4444argb.height + 10;
        gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_raw_4444argb);
        break;

    case ID_ALPHAMAP:
        gx_context_fill_color_set(window->selected_fill_color);

        xpos = size->left;
        ypos = size->top;
        gx_canvas_pixelmap_draw(xpos, ypos, &pixelmap_compress_alphamap);

        ypos = size->top + pixelmap_compress_alphamap.height + 10;
