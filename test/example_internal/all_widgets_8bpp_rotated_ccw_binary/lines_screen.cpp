...
#include <stdio.h>
#include "gx_api.h"
#include "all_widgets_8bpp_rotated_ccw_binary_resources.h"
#include "all_widgets_8bpp_rotated_ccw_binary_specifications.h"
...
VOID calculate_line_ends(VOID)
{
    ...
    INT x_center = (pLineWin->size.left + pLineWin->size.right) / 2;
    INT y_center = (pLineWin->size.top + pLineWin->size.bottom) / 2;
...
UINT main_event_handler(GX_WINDOW *window, GX_EVENT *myevent)
{
    ...
    case GX_SIGNAL(ID_BRUSH_ALPHA_SLIDER, GX_EVENT_SLIDER_VALUE):
        lines_screen_brush_alpha = (GX_UBYTE)myevent->payload.longdata;
...
GX_RECTANGLE *size;
    ...
    brush->alpha = lines_screen_brush_alpha;

    if (lines_screen_rounded)
    {
        brush_style |= GX_BRUSH_ROUND;
    }
    gx_context_brush_define(lines_screen_line_color, window ->normal_fill_color, brush_style);
    ...
    gx_canvas_line_draw(size->left + 5, size->top + 5, size->left + 5, size->bottom - 5);
    gx_context_brush_pattern_set(0xf0f0f0f0);
    gx_canvas_line_draw(size->left + 5, size->top + 5, size->right - 5, size->top + 5);
    gx_context_brush_pattern_set(0xaaaaaaaa);
    gx_canvas_line_draw(size->left + 5, size->bottom - 5, size->right - 5, size->bottom - 5);
    gx_context_brush_pattern_set(0x33333333);
    gx_canvas_line_draw(size->right - 5, size->top + 5, size->right - 5, size->bottom - 5);
