...
GX_CHAR binres_pathname[] = "all_widgets_8bpp_rotated_ccw_binary_resources.bin";
...
VOID  demo_thread_entry(ULONG thread_input)
{
    ...
    gx_studio_display_configure(MAIN_DISPLAY, win32_graphics_driver_setup_8bit_palette_rotated,
...
UINT load_binary_resource_data_to_ram()
{
    ...
    binres_root_address = (GX_UBYTE *)dynamic_memory_allocate(total_length);
...
UINT load_binres_theme(GX_UBYTE* root_address, INT theme_id)
{
    ...
    if (status == GX_SUCCESS)
    {
        ...
        gx_display_theme_install(root->canvas->display, new_theme);
    ...
    if (status == GX_SUCCESS)
    {
        ...
        gx_display_language_table_set_ext(root->canvas->display, (GX_CONST GX_STRING**)new_table, 1, MAIN_DISPLAY_STRING_TABLE_SIZE);
