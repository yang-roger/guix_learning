...
VOID test_line_chart_scale_draw(GX_LINE_CHART *chart)
{
    ...
    do {
        if ((scale_count % 4)  == 0)
        {
            ...
            str.ptr = ScaleString;
            str.length = strnlen(ScaleString, sizeof(ScaleString));
...
VOID test_line_chart_grid_draw(GX_LINE_CHART *chart)
{
    ...
    step /= NUM_VERTICAL_GRID_LINES;
    offset = GX_FIXED_VAL_MAKE(chart_bound.left) + step;

    pos = GX_FIXED_VAL_TO_INT(offset);
    gx_context_brush_width_set(1);
    gx_context_line_color_set(pressure_chart_info.grid_color);
    line_start = chart_bound.top;
    line_end = chart_bound.bottom - gx_info->axis_line_width;
    ...
    step = chart_bound.bottom - chart_bound.top + 1;
    step = GX_FIXED_VAL_MAKE(step);
    
    /* 7 gridlines? */
    step /= 7;
    offset = GX_FIXED_VAL_MAKE(chart_bound.bottom) - step;
    pos = GX_FIXED_VAL_TO_INT(offset);

    line_start = chart_bound.left + gx_info->axis_line_width;
    line_end = chart_bound.right;

    do {
        gx_canvas_line_draw(line_start, pos, line_end, pos);
        offset -= step;
        pos = GX_FIXED_VAL_TO_INT(offset);
    } while (pos > chart_bound.top);
}
...
VOID test_line_chart_reference_lines_draw(GX_LINE_CHART *chart)
{
    ...
    GX_LINE_CHART_INFO *gx_info = &chart->info;
    chart_bound = chart->size;
    chart_bound.right -= gx_info->right_margin;
    chart_bound.bottom -= gx_info->bottom_margin;
    ...
    line_start = chart_bound.left + gx_info->axis_line_width;
    line_end = chart->size.right;
    bottom_pos = chart_bound.bottom;
    gx_context_brush_width_set(1);
    gx_context_brush_get(&brush);
    brush->line_pattern = pressure_chart_info.dash_line_pattern;
    brush->pattern_mask = 0x80000000;

    // draw the high line
    y_pos = pressure_chart_info.high_line_value - chart->info.min_val;
    y_pos *= y_scale;
    ...
    y_pos -= label_font->line_height + 4;
    gx_context_line_color_set(pressure_chart_info.axis_label_color);
    str.ptr = "HIGH";
    str.length = 4;
    gx_canvas_text_draw_ext(chart_bound.right, y_pos, &str);

    // draw the med line
    y_pos = pressure_chart_info.med_line_value - chart->info.min_val;
    y_pos *= y_scale;
    y_pos = bottom_pos - GX_FIXED_VAL_TO_INT(y_pos);
    ...
    // draw the low line
    y_pos = pressure_chart_info.low_line_value - chart->info.min_val;
    y_pos *= y_scale;
    y_pos = bottom_pos - GX_FIXED_VAL_TO_INT(y_pos);
