...
#define            NESTED_WINDOW_WIDTH  97
#define            NESTED_WINDOW_HEIGHT  53
#define            NUMBER_ANIMATION_TIMER 2

#define            DROP_LIST_VISIBLE_ROWS 5
#define            SCRATCHPAD_PIXELS (DISPLAY_1_X_RESOLUTION * DISPLAY_1_X_RESOLUTION)
...
extern GX_STUDIO_DISPLAY_INFO all_widgets_332rgb_display_table[];
...
VOID  demo_thread_entry(ULONG thread_input)
{
    ...
    gx_studio_display_configure(DISPLAY_1, win32_graphics_driver_setup_332rgb,
    ...
    /* create menu screen */
    gx_studio_named_widget_create("line_screen", GX_NULL, (GX_WIDGET **)&pLineScreen);
    ...
    gx_canvas_create(&composite_canvas, GX_NULL,
        all_widgets_332rgb_display_table[0].display,
        GX_CANVAS_COMPOSITE,
    ...
    gx_window_root_create(&nested_root, "nested_root", &animation_canvas, GX_STYLE_BORDER_NONE, GX_NULL, &NestedWindow.size);
...
UINT next_button_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_SHOW:
        ...
        if (window->id == ID_WINDOW_SCREEN)
        {
            ...
            gx_canvas_offset_set(&animation_canvas, 72, 74);
            gx_widget_show((GX_WIDGET *)&nested_root);

            gx_window_client_scroll(&window_screen.window_screen_window_5, 1, 1);
        }
        break;
    ...
    case GX_EVENT_HIDE:
        ...
        else if (window->id == ID_TEXT_SCREEN)
        {
    ...
    case GX_SIGNAL(IDB_NEXT, GX_EVENT_CLICKED):
        ...
        case ID_TEXT_SCREEN:
            ToggleScreen(&sprite_screen, window);
            break;
        ...
        case ID_MENU_SCREEN:
            ToggleScreen(pLineScreen, window);
            break;
...
UINT pic_screen_handler(GX_WINDOW *window, GX_EVENT *myevent)
{
    UINT status = 0;
    switch (myevent->type)
    {
...
VOID pic_window_draw(GX_WINDOW *window)
{
    if (alpha)
    {
        if (compress)
        {
            gx_window_wallpaper_set(window, GX_PIXELMAP_ID_APPLE, GX_FALSE);
        }
        else
        {
            gx_window_wallpaper_set(window, GX_PIXELMAP_ID_FRAME_003, GX_FALSE);
        }
    }
    else
    {
        if (compress)
        {
            gx_window_wallpaper_set(window, GX_PIXELMAP_ID_FOOT, GX_FALSE);
        }
        else
        {
