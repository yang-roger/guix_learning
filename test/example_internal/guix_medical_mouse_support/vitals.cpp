...
GX_PIXELMAP *map;
    ...
    chart_update_rectangle.right = chart_update_rectangle.left + (CHART_SCROLL * 2) - 1;
    ...
    gx_canvas_pixelmap_draw(chart_area.left, chart_area.top, map);

    /* draw the most recent two lines
       one line is the newly drawn value, and the second
       line is re-drawn so that it is not clipped
    */
    gx_context_brush_width_set(CHART_LINE_WIDTH);
    gx_context_brush_style_set(GX_BRUSH_ROUND|GX_BRUSH_ALIAS);

    gx_canvas_line_draw(chart_update_rectangle.left + 1,
        chart_update_rectangle.top + line_y_coords[0],
        chart_update_rectangle.left + CHART_SCROLL,
        chart_update_rectangle.top + line_y_coords[1]);

    gx_canvas_line_draw(chart_update_rectangle.left + CHART_SCROLL,
        chart_update_rectangle.top + line_y_coords[1],
        chart_update_rectangle.left + (CHART_SCROLL * 2),
        chart_update_rectangle.top + line_y_coords[2]);
...
GX_ICON *heart_icon;
    ...
    gx_slider_value_set((GX_SLIDER *) &vitals_screen.vitals_screen_cardio_slider,
        &vitals_screen.vitals_screen_cardio_slider.info, slider_value);
    ...
    */
    ...
    if (ekg_value == 0)
    {
        if (warn_heart)
        {
            if (heart_icon->normal_pixelmap != GX_PIXELMAP_ID_ICON_HEART_RED_SMALL)
            {
        ...
        {
            if (heart_icon->normal_pixelmap != GX_PIXELMAP_ID_ICON_HEART_GRN_SMALL)
            {
    ...
    else
    {
        if (ekg_index >= 19 && ekg_index <= 22)
        {
            if (warn_heart)
            {
                if (heart_icon->normal_pixelmap != GX_PIXELMAP_ID_ICON_HEART_RED_LARGE)
                {
            ...
            {
                if (heart_icon->normal_pixelmap != GX_PIXELMAP_ID_ICON_HEART_GRN_LARGE)
                {
        ...
        else
        {
            if (warn_heart)
            {
                if (heart_icon->normal_pixelmap != GX_PIXELMAP_ID_ICON_HEART_RED_MED)
                {
            ...
            {
                if (heart_icon->normal_pixelmap != GX_PIXELMAP_ID_ICON_HEART_GRN_MED)
                {
...
GX_RECTANGLE block_rect;
    ...
    if (retrace_xpos + CHART_SCROLL > chart_area.right)
    {
        retrace_xpos = 0;
    }

    block_rect.top = chart_area.top;
    block_rect.bottom = chart_area.bottom;
    block_rect.left = chart_area.left + retrace_xpos;
    block_rect.right = chart_area.left + retrace_xpos + (CHART_SCROLL * 8);
    ...
    gx_canvas_line_draw(chart_area.left + retrace_xpos,
        chart_area.top + line_y_coords[1],
        chart_area.left + retrace_xpos + CHART_SCROLL,
        chart_area.top + line_y_coords[0]);

    gx_canvas_pixelmap_draw(chart_area.left + retrace_xpos + CHART_SCROLL,
        chart_area.top + line_y_coords[0] - ball->height / 2, ball);
...
VOID ToggleChartType(VOID)
{
    if (chart_type == CHART_TYPE_SCROLLING)
    {
        ...
        chart_area.left = vitals_screen.vitals_screen_waveform_window.client.left;
    ...
    else
    {
        ...
        chart_area.left = vitals_screen.vitals_screen_cardio_slider.size.right + 1;
        gx_widget_shift(&vitals_screen.vitals_screen_current_hr_prompt, 0, HR_PROMPT_VERTICAL_SHIFT, GX_TRUE);
...
VOID AnimateHRWin(VOID)
{
    ...
    left = vitals_screen.vitals_screen_hr_win.size.left;
    stop = vitals_screen.vitals_screen_waveform_window.size.right + 5;
...
GX_WIDGET *widget = (GX_WIDGET *) window;
    ...
    case GX_EVENT_SHOW:
        ...
        ekg_index = 0;
        chart_area = vitals_screen.vitals_screen_waveform_window.client;
        ...
        gx_widget_shift(&vitals_screen.vitals_screen_hr_win,
            vitals_screen.size.right - vitals_screen.vitals_screen_hr_win.size.left, 0, GX_TRUE);

        retrace_xpos = chart_area.left;
    ...
    case GX_EVENT_PEN_DOWN:
        if (gx_utility_rectangle_point_detect(&vitals_screen.vitals_screen_hr_win.size, myevent->payload.pointdata) ||
            gx_utility_rectangle_point_detect(&vitals_screen.vitals_screen_waveform_window.size, myevent->payload.pointdata))
        {
            ToggleChartType();
        }
        break;
    ...
    case GX_EVENT_TIMER:
        switch(myevent ->payload.timer_id)
        {
        case CHART_TIMER:
            if (chart_type == CHART_TYPE_SCROLLING)
            {
