...
INT value;
    ...
    case GX_SIGNAL(ID_SLIDER1, GX_EVENT_SLIDER_VALUE):
        ...
        update_prompt(prompt, value_strings[0], myevent->payload.longdata);

        if (!animation_running)
        {
            med_levels[0] = myevent->payload.longdata;
        }
        break;

    case GX_SIGNAL(ID_SLIDER2, GX_EVENT_SLIDER_VALUE):
        gx_widget_find(window, ID_VAL2_DISPLAY, 0, (GX_WIDGET **) &prompt);
        update_prompt(prompt, value_strings[1], myevent->payload.longdata);
        if (!animation_running)
        {
            med_levels[1] = myevent->payload.longdata;
        }
        break;

    case GX_SIGNAL(ID_SLIDER3, GX_EVENT_SLIDER_VALUE):
        gx_widget_find(window, ID_VAL3_DISPLAY, 0, (GX_WIDGET **) &prompt);
        update_prompt(prompt, value_strings[2], myevent->payload.longdata);

        if (!animation_running)
        {
            med_levels[2] = myevent->payload.longdata;
        }
        break;

    case GX_SIGNAL(ID_SLIDER4, GX_EVENT_SLIDER_VALUE):
        gx_widget_find(window, ID_VAL4_DISPLAY, 0, (GX_WIDGET **) &prompt);
        update_prompt(prompt, value_strings[3], myevent->payload.longdata);

        if (!animation_running)
        {
            med_levels[3] = myevent->payload.longdata;
        }
        break;
    ...
    case GX_EVENT_TIMER:
        ...
        if (animation_step == NUM_ANIMATION_STEPS)
        {
            ...
            {
                slider = med_sliders[loop];
                gx_slider_value_set(slider, &slider->info, med_levels[loop]);
            }
        ...
        else
        {
            ...
            {
                slider = med_sliders[loop];
                value = (med_levels[loop] + slider->info.current_val) / 2;
                gx_slider_value_set(slider, &slider->info, value);
            }
        }
        break;
...
GX_STRING text;
    ...
    gx_context_font_get(prompt->font_id_(), &font);
    ...
    gx_canvas_pixelmap_draw(prompt->size.left + twidth,
                            prompt->size.top, map);
}