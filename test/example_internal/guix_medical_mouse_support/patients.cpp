...
{
    return patient_list[selected_patient_index].name;
}

GX_CHAR *GetPatientAdmitDate(void)
{
    return patient_list[selected_patient_index].admit_date;
}
...
void patient_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    if (!result)
    {
        gx_utility_rectangle_define(&childsize, list->client.left + 10, 0,
                                                list->client.right - 10, 27);
        ...
        childsize.left += 10;
        childsize.right = childsize.left + 190;
        ...
        gx_prompt_text_color_set(&row->name, GX_COLOR_ID_OFF_WHITE, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK);
        ...
        gx_prompt_text_color_set(&row->mr_num, GX_COLOR_ID_OFF_WHITE, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK);
        ...
        gx_prompt_text_color_set(&row->room_num, GX_COLOR_ID_OFF_WHITE, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK);
        ...
        gx_prompt_text_color_set(&row->admit_date, GX_COLOR_ID_OFF_WHITE, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK);
    }
    gx_widget_fill_color_set(&row->background, bkgnd_color, GX_COLOR_ID_GREEN, GX_COLOR_ID_GREEN);
    gx_prompt_text_set(&row->name, patient_list[index].name);
    gx_prompt_text_set(&row->mr_num, patient_list[index].mr_num);
    gx_prompt_text_set(&row->room_num, patient_list[index].room_num);
    gx_prompt_text_set(&row->admit_date, patient_list[index].admit_date);
}
...
PATIENTS_SCREEN_CONTROL_BLOCK *screen = (PATIENTS_SCREEN_CONTROL_BLOCK *) window;
    ...
    case GX_SIGNAL(ID_PATIENT_LIST, GX_EVENT_LIST_SELECT):
        ...
        myevent->type = GX_SIGNAL(ID_MEDS_BUTTON, GX_EVENT_CLICKED);
        break;
    }
