...
#define ID_ANIMATION_CANVAS_ROOT_WINDOW 65500
...
VOID tx_application_define(void *first_unused_memory)
{
    ...
    tx_byte_pool_create(&memory_pool, "scratchpad", scratchpad, SCRATCHPAD_PIXELS * sizeof(GX_COLOR));
...
VOID start_guix(void)
{
    ...
    gx_canvas_memory_define(root->canvas, (GX_COLOR*)canvas_buffer, sizeof(canvas_buffer));
    ...
    gx_studio_named_widget_create("line_screen", GX_NULL, GX_NULL);

    gx_studio_named_widget_create("pic_screen", GX_NULL, GX_NULL);
    ...
    winsize.right = NESTED_WINDOW_WIDTH - 1;
    winsize.bottom = NESTED_WINDOW_HEIGHT - 1;
    ...
    gx_window_root_create(&nested_root, "nested_root", &animation_canvas, GX_STYLE_BORDER_NONE, ID_ANIMATION_CANVAS_ROOT_WINDOW, &NestedWindow.size);
...
void scroll_wheel_value_set(GX_PROMPT *prompt, INT selected_row)
{
    ...
    gx_utility_ltoa(selected_row, text_buffer, sizeof(text_buffer));
...
UINT buttons_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_BUTTON, GX_EVENT_CLICKED):
        button_click_count++;
        gx_numeric_prompt_value_set(&button_screen.button_screen_count_prompt, button_click_count);
        break;
    ...
    case GX_SIGNAL(ID_BUTTON, GX_EVENT_FOCUS_GAIN_NOTIFY):
        button_focus_gain_count++;
        break;
...
UINT text_screen_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_SL_INPUT, GX_EVENT_TEXT_EDITED):
        gx_single_line_text_input_buffer_get(&text_screen.text_screen_text_input_1, (GX_CHAR **)&string.ptr, &string.length, &buffer_size);
        if ((string.length == text_screen.text_screen_text_input_1.string_size) &&
            (buffer_size == text_screen.text_screen_text_input_1.buffer_size))
        {
            gx_multi_line_text_view_text_set_ext(&text_screen.text_screen_text_view_1, &string);
        }
        break;

    case GX_SIGNAL(ID_ML_INPUT, GX_EVENT_TEXT_EDITED):
        gx_multi_line_text_input_buffer_get(&text_screen.text_screen_text_input_2, (GX_CHAR **)&string.ptr, &string.length, &buffer_size);
        if ((string.length == text_screen.text_screen_text_input_2.text_().length) &&
            (buffer_size == text_screen.text_screen_text_input_2.buffer_size))
        {
            gx_multi_line_text_view_text_set_ext(&text_screen.text_screen_text_view_1, &string);
        }
        break;
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == NUMBER_ANIMATION_TIMER)
        {
...
UINT radial_slider_screen_event_process(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    switch (event_ptr->type)
    {
    case GX_SIGNAL(ID_ANIMATED, GX_EVENT_TOGGLE_ON):
        gx_radial_slider_animation_set(&radial_slider_screen.radial_slider_screen_radial_slider, 15, 2, GX_ANIMATION_CIRC_EASE_IN_OUT, GX_NULL);
        break;
    ...
    case GX_SIGNAL(ID_SET_ANCHOR_LIST, GX_EVENT_TOGGLE_OFF):
        gx_radial_slider_anchor_angles_set(&radial_slider_screen.radial_slider_screen_radial_slider, GX_NULL, 0);
        break;

    case GX_SIGNAL(ID_RADIAL_SLIDER, GX_EVENT_SLIDER_VALUE):
        gx_numeric_prompt_value_set(&radial_slider_screen.radial_slider_screen_radial_slider_value, event_ptr->payload.intdata[0]);
        break;
...
UINT glyph_screen_event_process(GX_WINDOW* widget, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_FONT_4BPP, GX_EVENT_RADIO_SELECT):
        gx_multi_line_text_view_font_set(&glyph_screen.glyph_screen_view, GX_FONT_ID_FONT_4BPP);
        break;

    case GX_SIGNAL(ID_FONT_1BPP, GX_EVENT_RADIO_SELECT):
        gx_multi_line_text_view_font_set(&glyph_screen.glyph_screen_view, GX_FONT_ID_FONT_1BPP);
...
GX_STRING string;
    ...
    gx_utility_ltoa(index + 1, temp, sizeof(temp));
    ...
    string.ptr = entry->text;
    string.length = strnlen(entry->text, sizeof(entry->text));
...
GX_STRING text;
    ...
    gx_context_string_get_ext(GX_STRING_ID_STRING_44, &text);
    gx_context_brush_define(widget->normal_text_color_(), 0, GX_BRUSH_UNDERLINE);
    gx_context_brush_width_set(2);
    gx_context_font_set(widget->font_id_());
    gx_canvas_text_draw_ext(widget->size.left,
                            widget->size.top,
                            &text);
}
...
{
    gx_utility_ltoa(value % 1000, prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE);
}
