...
INT               shape_screen_alpha = 0;
INT               shape_screen_compress = 0;
INT               fill_pixelmap_index = 0;

GX_RESOURCE_ID  fill_pixelmap_id[4] = {GX_PIXELMAP_ID__565RGB, GX_PIXELMAP_ID__565RGB_COMPRESS, GX_PIXELMAP_ID__565RGB_ALPHA, GX_PIXELMAP_ID__565RGB_COMPRESS_ALPHA};
...
GX_WINDOW *pGraphicsWin = &shapes_screen.shapes_screen_graphics_window;
    ...
    case GX_SIGNAL(ID_BRUSH_ALPHA, GX_EVENT_SLIDER_VALUE):
        ...
        gx_numeric_prompt_value_set(&shapes_screen.shapes_screen_brush_alpha, brush_alpha);
        gx_system_dirty_mark(pGraphicsWin);
        break;

    case GX_SIGNAL(ID_RADIUS_SLIDER, GX_EVENT_SLIDER_VALUE):
        radius = myevent->payload.longdata;
        gx_numeric_prompt_value_set(&shapes_screen.shapes_screen_radius, radius);
        gx_system_dirty_mark(pGraphicsWin);
        break;

    case GX_SIGNAL(ID_BRUSH_WIDTH, GX_EVENT_SLIDER_VALUE):
        brush_width = myevent->payload.longdata;
        gx_numeric_prompt_value_set(&shapes_screen.shapes_screen_brush_width, brush_width);
        gx_system_dirty_mark(pGraphicsWin);
        break;
    ...
    case GX_EVENT_TIMER:
        if (myevent->payload.timer_id == ARC_TIMER)
        {
        ...
        else if (myevent->payload.timer_id == PIE_TIMER)
        {
    ...
    fill_pixelmap_index = shape_screen_alpha + shape_screen_compress;
    return 0;
}
...
VOID graphics_draw(GX_WINDOW *window)
{
    ...
    if (pixelmap_fill)
    {
        ...
        gx_context_pixelmap_set(fill_pixelmap_id[fill_pixelmap_index]);
    }
    ...
    case PIE:
        gx_context_brush_define(GX_COLOR_ID_GRAY, GX_COLOR_ID_RED, brush_style);
