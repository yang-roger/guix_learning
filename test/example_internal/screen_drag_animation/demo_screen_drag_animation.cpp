...
UINT h_menu_window_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_SHOW:
        ...
        info.parent = (GX_WIDGET *)window;
        info.style = h_screen_drag_style;
        info.id = ANIMATION_ID_H_SCREEN_DRAG;
        info.frame_interval = 1;
        info.steps = 20;
        info.slide_screen_list = h_screen_list;
...
UINT v_menu_window_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_SHOW:
        memset(&info, 0, sizeof(GX_ANIMATION_INFO));
        info.parent = (GX_WIDGET *)window;
        info.style = v_screen_drag_style;
        info.id = ANIMATION_ID_V_SCREEN_DRAG;
        info.frame_interval = 1;
        info.steps = 20;
        info.slide_screen_list = v_screen_list;
...
VOID drop_list_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    /* Set prompt text. */
    str.ptr = (GX_CHAR *)easing_function_list[index].type_text;
    str.length = strnlen((GX_CHAR *)easing_function_list[index].type_text, 30);
