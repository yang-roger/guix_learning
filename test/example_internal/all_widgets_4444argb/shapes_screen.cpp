...
INT               compress = 0;
INT               alpha = 0;
INT               fill_pixelmap_id = 0;

GX_RESOURCE_ID  pixelmap_id[] = {GX_PIXELMAP_ID_ID_RAW, GX_PIXELMAP_ID_ID_COMPRESS, GX_PIXELMAP_ID_ID_ALPHA, GX_PIXELMAP_ID_ID_COMPRESS_ALPHA};
...
{
    static GX_CHAR string_buffer[10];
    GX_STRING string;
    ...
    if (pp)
    {
        ...
        string.length = strnlen(string_buffer, sizeof(string_buffer));
...
GX_WINDOW *pGraphicsWin = &(((SHAPES_SCREEN_CONTROL_BLOCK *)pShapesScreen)->shapes_screen_graphics_window);
    ...
    fill_pixelmap_id = alpha + compress;
    return status;
}
...
VOID graphics_draw(GX_WINDOW *window)
{
    ...
    if (pixelmap_fill)
    {
        ...
        gx_context_pixelmap_set(pixelmap_id[fill_pixelmap_id]);
    }
    ...
    {
        gx_context_raw_line_color_set(0xff6A6D6B);
