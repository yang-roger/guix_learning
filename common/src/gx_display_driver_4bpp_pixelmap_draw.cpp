...
GX_RECTANGLE   *clip = context->clip;
    ...
    offset_row = (GX_VALUE)(clip->left - xpos);
    getstride = (pixelmap->width + 1) >> 1;
    getrow = (const GX_UBYTE *)pixelmap->data;
    getrow += getstride * (clip->top - ypos);

    getauxstride = (pixelmap->width + 7) >> 3;
    getauxrow = (const GX_UBYTE *)pixelmap->aux_data;
    getauxrow += getauxstride * (clip->top - ypos);
...
GX_RECTANGLE   *clip = context->clip;
    ...
    putstride = (context->canvas->x_resolution + 1) >> 1;
    getmask = 0xf0;
                                           ...
                                           INT xpos, INT ypos, GX_PIXELMAP *pixelmap)
{
    ...
    if (pixelmap->format != GX_COLOR_FORMAT_4BIT_GRAY ||
