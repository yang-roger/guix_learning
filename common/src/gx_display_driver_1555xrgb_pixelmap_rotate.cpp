...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_display_driver_1555xrgb_pixelmap_raw_rotate                     */
/*                                                           6.1.10       */
/*  AUTHOR                                                                */
/*                                                                        */
    ...
    for (y = clip->top - newypos; y <= clip->bottom - newypos; y++)
    {
        for (x = clip->left - newxpos; x <= clip->right - newxpos; x++)
        {
                ...
                (yy >= -1) && (yy < pixelmap->height))
            {
                ...
                pixel = gx_color_1555xrgb_raw_from_4colors_2(a, b, c, d, xdiff, ydiff, alpha);

                blend_func(context, x + newxpos, y + newypos, (GX_COLOR)pixel, (GX_UBYTE)alpha);
            }
        }
    }
}
        ...
        for (x = clip->left - newxpos; x <= clip->right - newxpos; x++)
        {
                ...
                (yy >= -1) && (yy < pixelmap->height))
            {
                    ...
                    (yy >= 0) && (yy < pixelmap->height - 1))
                {
                    ...
                    c = *(get + pixelmap->width);
                    alpha[2] = *(getalpha + pixelmap->width);

                    d = *(get + pixelmap->width + 1);
                    alpha[3] = *(getalpha + pixelmap->width + 1);
                }
                else
                {
                    get = (USHORT *)pixelmap->data;
                    ...
                    if (xx == -1)
                    {
                        ...
                        {
                            b = *(get + yy * pixelmap->width);
                            alpha[1] = *(getalpha + yy * pixelmap->width);
                        }

                        if (yy < pixelmap->height - 1)
                        {
                            d = *(get + (yy + 1) * pixelmap->width);
                            alpha[3] = *(getalpha + (yy + 1) * pixelmap->width);
                        }
                    }
                    else if (yy == -1)
                    {
                    ...
                    else if (xx == pixelmap->width - 1)
                    {
                        /* handle right edget. */
                        a = *(get + yy * pixelmap->width + xx);
                        alpha[0] = *(getalpha + yy * pixelmap->width + xx);

                        if (yy < pixelmap->height - 1)
                        {
                            c = *(get + (yy + 1) * pixelmap->width + xx);
                            alpha[2] = *(getalpha + (yy + 1) * pixelmap->width + xx);
                        }
                    }
                    else
                    {
                        ...
                        b = *(get + yy * pixelmap->width + xx + 1);
                        alpha[1] = *(getalpha + yy * pixelmap->width + xx + 1);
                    }
                ...
                pixel = gx_color_1555xrgb_alpha_from_4colors_2(a, b, c, d, xdiff, ydiff,
                                                               alpha[0], alpha[1], alpha[2], alpha[3]);

                blend_func(context, x + newxpos, y + newypos, (GX_COLOR)pixel, (GX_UBYTE)alpha[0]);
            }
        }
    }
}
