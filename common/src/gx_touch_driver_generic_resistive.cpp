...
{
    GX_EVENT event;
    event.type = GX_EVENT_PEN_DOWN;
    event.payload.pointdata = touch->current_touch_coord;
    event.sender = 0;
    event.target = 0;
    event.display_handle = 0;
    _gx_system_event_send(&event);
    touch->last_touch_coord = touch->current_touch_coord;
    touch->last_touch_state = GX_TOUCH_STATE_TOUCHED;
}
...
{
    GX_EVENT event;
    INT x_delta = GX_ABS(touch->current_touch_coord.x - touch->last_touch_coord.x);
    INT y_delta = GX_ABS(touch->current_touch_coord.y - touch->last_touch_coord.y);

    if (x_delta > touch->min_drag_delta ||
        y_delta > touch->min_drag_delta)
    {
        event.type = GX_EVENT_PEN_DRAG;
        event.payload.pointdata = touch->current_touch_coord;
        event.sender = 0;
        event.target = 0;
        event.display_handle = 0;
...
static void _gx_touch_driver_generic_resistive_pen_up_event_send(GX_RESISTIVE_TOUCH *touch)
{
    ...
    touch->last_touch_coord = touch->current_touch_coord;
    _gx_system_event_send(&event);
    touch->last_touch_state = GX_TOUCH_STATE_RELEASED;
}
                                                               ...
                                                               GX_POINT *raw_coord)
{
    ...
    TOUCH_CALIBRATION_MATRIX *matrix_ptr = &touch->calibration_matrix;

    if (matrix_ptr->Divider != 0)
    {
        touch->current_touch_coord.x =
                        (GX_VALUE) (((matrix_ptr->An * raw_coord->x) +
                          (matrix_ptr->Bn * raw_coord->y) +
                           matrix_ptr->Cn) / matrix_ptr->Divider);

        touch->current_touch_coord.y =
                        (GX_VALUE) (((matrix_ptr->Dn * raw_coord->x) +
                          (matrix_ptr->En * raw_coord->y) +
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_touch_driver_generic_resistive_setup                            */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
void _gx_touch_driver_generic_resistive_setup(GX_RESISTIVE_TOUCH *touch, GX_RESISTIVE_TOUCH_INFO *info)
{
    ...
    memcpy(&touch->info, info, sizeof(GX_RESISTIVE_TOUCH_INFO)); /* Use case of memcpy is verified. */
}
                                                                     ...
                                                                     GX_POINT *touch_points)
{
    ...
    matrix_ptr->Divider = ((touch_points[0].x - touch_points[2].x) * (touch_points[1].y - touch_points[2].y)) -
        ((touch_points[1].x - touch_points[2].x) * (touch_points[0].y - touch_points[2].y)) ;
    ...
    {
        matrix_ptr->An = ((display_points[0].x - display_points[2].x) * (touch_points[1].y - touch_points[2].y)) -
                        ((display_points[1].x - display_points[2].x) * (touch_points[0].y - touch_points[2].y)) ;

        matrix_ptr->Bn = ((touch_points[0].x - touch_points[2].x) * (display_points[1].x - display_points[2].x)) -
                        ((display_points[0].x - display_points[2].x) * (touch_points[1].x - touch_points[2].x)) ;

        matrix_ptr->Cn = (touch_points[2].x * display_points[1].x - touch_points[1].x * display_points[2].x) * touch_points[0].y +
                        (touch_points[0].x * display_points[2].x - touch_points[2].x * display_points[0].x) * touch_points[1].y +
                        (touch_points[1].x * display_points[0].x - touch_points[0].x * display_points[1].x) * touch_points[2].y ;

        matrix_ptr->Dn = ((display_points[0].y - display_points[2].y) * (touch_points[1].y - touch_points[2].y)) -
                        ((display_points[1].y - display_points[2].y) * (touch_points[0].y - touch_points[2].y)) ;

        matrix_ptr->En = ((touch_points[0].x - touch_points[2].x) * (display_points[1].y - display_points[2].y)) -
                        ((display_points[0].y - display_points[2].y) * (touch_points[1].x - touch_points[2].x)) ;

        matrix_ptr->Fn = (touch_points[2].x * display_points[1].y - touch_points[1].x * display_points[2].y) * touch_points[0].y +
                        (touch_points[0].x * display_points[2].y - touch_points[2].x * display_points[0].y) * touch_points[1].y +
                        (touch_points[1].x * display_points[0].y - touch_points[0].x * display_points[1].y) * touch_points[2].y ;
    }
...
static GX_BOOL _gx_touch_driver_generic_resistive_raw_read(GX_RESISTIVE_TOUCH *touch, GX_POINT *ret_val)
{
    ...
    samples = touch->sample_ram;
    read_function = touch->sample_read;
    stability_limit = touch->stability_limit;
    sample_size = touch->sample_size;
    max_retries = touch->max_retries;
    ...
    while(!stable_sample)
    {
        ...
        if (retries < max_retries)
        {
            ...
            {
                samples[loop].x = (GX_VALUE) read_function(GX_TOUCH_X_AXIS);
                samples[loop].y = (GX_VALUE) read_function(GX_TOUCH_Y_AXIS);

                if (loop > 0)
                {
                    delta_x = GX_ABS(samples[loop].x - samples[loop - 1].x);
                    if (delta_x > stability_limit)
                    {
                        stable_sample = GX_FALSE;
                        break;
                    }
                    delta_y = GX_ABS(samples[loop].y - samples[loop - 1].y);
    ...
    if (stable_sample)
    {
        ...
        {
            delta_x += samples[loop].x;
            delta_y += samples[loop].y;
        }
        ret_val->x = (GX_VALUE) (delta_x / sample_size);
        ret_val->y = (GX_VALUE) (delta_y / sample_size);
    }
...
static GX_BOOL _gx_touch_driver_generic_resistive_sample_read(GX_RESISTIVE_TOUCH *touch, GX_POINT *ret_val)
{
    ...
    {
        ret_val->x = (GX_VALUE) ((raw_val.x + 2 ) / 4);
        ret_val->y = (GX_VALUE) ((raw_val.y + 2 ) / 4);
        return GX_TRUE;
    }
    return GX_FALSE;
}
...
void _gx_touch_driver_generic_resistive_update(GX_RESISTIVE_TOUCH *touch)
{
    ...
    {
        if (touch->current_touch_state == GX_TOUCH_STATE_TOUCHED)
        {
            if (touch->last_touch_state == GX_TOUCH_STATE_TOUCHED)
            {
...
void _gx_touch_driver_generic_resistive_calibrate(GX_RESISTIVE_TOUCH *touch)
{
    ...
    GX_MULTI_LINE_TEXT_VIEW *text = touch->info.text;
    GX_WIDGET *icon = (GX_WIDGET *) touch->info.target;
    pen_down = touch->pen_down_detect;

    icon_size = icon->size.right - icon->size.left + 1;

    display_cal_points[0].x = (GX_VALUE) (icon_size / 2);
    display_cal_points[0].y = (GX_VALUE) (icon_size / 2);
    display_cal_points[1].x = (GX_VALUE) (touch->x_range - (icon_size / 2));
    display_cal_points[1].y = (GX_VALUE) (touch->y_range / 2);
    display_cal_points[2].x = (GX_VALUE) (icon_size / 2);
    display_cal_points[2].y = (GX_VALUE) (touch->y_range - (icon_size / 2));

    if (pen_down())
    {
        _gx_widget_hide(icon);
        _gx_multi_line_text_view_text_id_set(text, touch->info.string_id_no_touch);
        _gx_system_canvas_refresh();
        _gx_touch_driver_generic_resistive_wait_for_no_touch(touch);

        _gx_multi_line_text_view_text_id_set(text, touch->info.string_id_touch_1);
    ...
    _gx_widget_shift(icon,
                    (GX_VALUE) (touch->x_range - icon_size),
                    (GX_VALUE) ((touch->y_range / 2) - (icon_size / 2)),
                     GX_FALSE);

    _gx_widget_show(icon);
    _gx_multi_line_text_view_text_id_set(text, touch->info.string_id_touch_2);
    _gx_system_canvas_refresh();
    ...
    newsize.left = (GX_VALUE) 0;
    newsize.bottom = (GX_VALUE) (touch->y_range);
    newsize.top = (GX_VALUE) (newsize.bottom - icon_size + 1);
    newsize.right = (GX_VALUE) (newsize.left + icon_size - 1);
    _gx_widget_resize(icon, &newsize);
    _gx_widget_show(icon);
    _gx_multi_line_text_view_text_id_set(text, touch->info.string_id_touch_3);
    _gx_system_canvas_refresh();
    ...
    calibration_event.type = GX_EVENT_TOUCH_CALIBRATION_COMPLETE;
    calibration_event.target = icon;
