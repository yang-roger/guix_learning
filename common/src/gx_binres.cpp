...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_resource_header_load(GX_BINRES_DATA_INFO *info, GX_RESOURCE_HEADER *header)
{
...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_theme_header_load(GX_BINRES_DATA_INFO *info, GX_THEME_HEADER *header)
{
    GX_BINRES_READ_USHORT(header->magic_number, info->root_address + info->read_offset);
    info->read_offset += sizeof(USHORT);
...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_color_header_load(GX_BINRES_DATA_INFO *info, GX_COLOR_HEADER *header)
{
    GX_BINRES_READ_USHORT(header->magic_number, info->root_address + info->read_offset);
    info->read_offset += sizeof(USHORT);
...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_palette_header_load(GX_BINRES_DATA_INFO *info, GX_PALETTE_HEADER *header)
{
    GX_BINRES_READ_USHORT(header->magic_number, info->root_address + info->read_offset);
    info->read_offset += sizeof(USHORT);
...
static UINT _gx_binres_font_header_load(GX_BINRES_DATA_INFO *info, GX_FONT_HEADER *header)
{
    ...
    GX_BINRES_READ_GX_UBYTE(header->built_in, info->root_address + info->read_offset);
    info->read_offset += sizeof(GX_UBYTE);
...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_glyph_header_load(GX_BINRES_DATA_INFO *info, GX_GLYPH_HEADER *header)
{
    GX_BINRES_READ_USHORT(header->magic_number, info->root_address + info->read_offset);
    info->read_offset += sizeof(USHORT);
...
static void _gx_binres_kerning_glyph_header_load(GX_BINRES_DATA_INFO *info, GX_KERNING_GLYPH_HEADER *header)
{
...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_string_header_load(GX_BINRES_DATA_INFO *info, GX_STRING_HEADER *header)
{
    GX_BINRES_READ_USHORT(header->magic_number, info->root_address + info->read_offset);
    info->read_offset += sizeof(USHORT);
...
/*                                                                        */
/**************************************************************************/
static void _gx_binres_language_header_load(GX_BINRES_DATA_INFO *info, GX_LANGUAGE_HEADER *header)
{
    GX_BINRES_READ_USHORT(header->magic_number, info->root_address + info->read_offset);
    info->read_offset += sizeof(USHORT);
...
static inline void _gx_binres_data_restart(GX_BINRES_DATA_INFO *info)
{
    /* Read Resource header. */
    info->read_offset = 0;
    _gx_binres_resource_header_load(info, &info->header);
}

static UINT _gx_binres_data_info_initialize(GX_BINRES_DATA_INFO *info, GX_UBYTE *root_address)
{
    memset(info, 0, sizeof(GX_BINRES_DATA_INFO));

    info->root_address = root_address;

    _gx_binres_data_restart(info);

    if (info->header.magic_number != GX_MAGIC_NUMBER)
    {
        return GX_INVALID_FORMAT;
    }
...
static UINT _gx_binres_data_string_header_seek(GX_BINRES_DATA_INFO *info, GX_STRING_HEADER *string_header)
{
    /* Skip theme info.  */
    info->read_offset += info->header.theme_data_size;
...
UINT _gx_binres_language_count_get(GX_UBYTE *root_address, GX_VALUE *put_count)
{
    ...
    status = _gx_binres_data_info_initialize(&info, root_address);
    if (status != GX_SUCCESS)
    {
        return status;
    }

    status = _gx_binres_data_string_header_seek(&info, &string_header);
    if (status != GX_SUCCESS)
    {
        return status;
    }
...
static UINT _gx_binres_language_table_buffer_allocate(GX_BINRES_DATA_INFO *info, GX_UBYTE language_table_type_size, GX_UBYTE string_table_type_size)
{
    ...
    if (_gx_binres_data_string_header_seek(info, &string_header) != GX_SUCCESS)
    {
        return GX_INVALID_FORMAT;
    }
...
UINT _gx_binres_language_table_load_ext(GX_UBYTE *root_address, GX_STRING ***returned_language_table)
{
    ...
    *returned_language_table = GX_NULL;
    ...
    _gx_binres_data_restart(&info);
    _gx_binres_data_string_header_seek(&info, &string_header);
    ...
    for (lang_index = 0; lang_index < string_header.language_count; lang_index++)
    {
        ...
        for (string_index = 1; string_index < string_header.string_count; string_index++)
        {
            ...
            GX_BINRES_READ_USHORT(string_length, get_data);
            info.read_offset += sizeof(USHORT);

            string = &language_table[lang_index][string_index];

            if (string_length)
            {
                string->ptr = (const GX_CHAR *)(info.root_address + info.read_offset);
            }
            else
            {
                string->ptr = GX_NULL;
...
static UINT _gx_binres_color_table_load(GX_BINRES_DATA_INFO *info, USHORT color_count, GX_COLOR **returned_color_table)
{
    ...
    _gx_binres_color_header_load(info, &color_header);

    if (color_header.magic_number != GX_MAGIC_NUMBER)
    {
        return GX_INVALID_FORMAT;
    }

    color_table = (GX_COLOR *)(info->root_address + info->read_offset);
    info->read_offset += color_header.data_size;
...
{
    GX_PALETTE_HEADER palette_header;
    ...
    _gx_binres_palette_header_load(info, &palette_header);

    if (palette_header.magic_number != GX_MAGIC_NUMBER)
    {
        return GX_INVALID_FORMAT;
    }

    palette_table = (GX_COLOR *)(info->root_address + info->read_offset);
    info->read_offset += palette_header.data_size;
...
{
    GX_GLYPH_HEADER glyph_header;
    GX_GLYPH       *glyph;
    USHORT          index;

    glyph = (GX_GLYPH *)(info->buffer + info->buffer_index);
    ...
    *returned_glyphs = glyph;
    ...
    {
        _gx_binres_glyph_header_load(info, &glyph_header);

        if (glyph_header.magic_number != GX_MAGIC_NUMBER)
        {
        ...
        glyph->ascent = glyph_header.ascent;
        glyph->descent = glyph_header.descent;
        glyph->advance = glyph_header.advance;
        glyph->leading = glyph_header.leading;
        glyph->width = glyph_header.width;
        glyph->height = glyph_header.height;

        /* Read glyph data.  */
        if (glyph_header.data_size)
        {
            glyph->map = (GX_UBYTE *)(info->root_address + glyph_header.map_offset);
            info->read_offset += glyph_header.data_size;
        }
        else
        {
            glyph->map = GX_NULL;
...
{
    GX_KERNING_GLYPH_HEADER glyph_header;
    GX_KERNING_GLYPH       *glyph;
    USHORT                  index;

    glyph = (GX_KERNING_GLYPH *)(info->buffer + info->buffer_index);
    ...
    *returned_glyphs = glyph;
    ...
    for (index = 0; index < glyph_count; index++)
    {
        _gx_binres_kerning_glyph_header_load(info, &glyph_header);
        ...
        if (glyph_header.kerning_table_size)
        {
            glyph->kerning_table = (GX_UBYTE *)(info->root_address + info->read_offset);
            info->read_offset += glyph_header.kerning_table_size;
        }
...
{
    GX_GLYPH_HEADER      glyph_header;
    GX_COMPRESSED_GLYPH *glyph;
    USHORT               index = 0;

    glyph = (GX_COMPRESSED_GLYPH *)(info->buffer + info->buffer_index);
    ...
    *returned_glyphs = glyph;
    ...
    for (index = 0; index < glyph_count; index++)
    {
        ...
        glyph->height = glyph_header.height;
        glyph->leading = glyph_header.leading;
        glyph->map_size = glyph_header.map_size;
        glyph->width = glyph_header.width;
...
static UINT _gx_binres_theme_seek(GX_BINRES_DATA_INFO *info,
                                  INT theme_id,
                                  GX_THEME_HEADER *theme_header)
{
    INT index;

    for (index = 0; index < info->header.theme_count; index++)
    {
...
static UINT _gx_binres_theme_buffer_allocate(GX_BINRES_DATA_INFO *info, INT theme_id)
{
    ...
    status = _gx_binres_theme_seek(info, theme_id, &theme_header);
    if (status != GX_SUCCESS)
    {
        return status;
    }
...
UINT _gx_binres_theme_load(GX_UBYTE *root_address, INT theme_id, GX_THEME **returned_theme)
{
    ...
    *returned_theme = GX_NULL;
    ...
    /* Allocate memory that needed for theme.  */
    ...
    _gx_binres_data_restart(&info);
    _gx_binres_theme_seek(&info, theme_id, &theme_header);
    ...
    theme->vertical_scrollbar_style = theme_header.hscroll_style;
