...
GUIX_LINKED_EVENT *pTest;
    ...
    while (pTest)
    {
        if (pTest->event_data.type == event_ptr->type)
        {
            /* found matching event, update it and return */
            pTest->event_data.payload.ulongdata = event_ptr->payload.ulongdata;
            OSMutexPost(&guix_event_queue.lock, OS_OPT_POST_NONE, &uc_error);
            return GX_SUCCESS;
        }
        pTest = pTest->next;
...
UINT   gx_generic_event_pop(GX_EVENT *put_event, GX_BOOL wait)
{
    ...
    *put_event = guix_event_queue.first->event_data;
    ...
    if (put_event->type == GX_EVENT_TIMER)
    {
        guix_timer_event_pending = GX_FALSE;
    }
...
GUIX_LINKED_EVENT *pTest;
    ...
    while (pTest)
    {
        ...
        if (pTest ->event_data.target)
        {
            if (pTest->event_data.target == target)
            {
                Purge = GX_TRUE;
            }
            else
            {
                gx_widget_child_detect(target, pTest->event_data.target, &Purge);
            }
            if (Purge)
            {
                pTest ->event_data.target = GX_NULL;
                pTest->event_data.type = 0;
            }
        }
        pTest = pTest->next;
    }
