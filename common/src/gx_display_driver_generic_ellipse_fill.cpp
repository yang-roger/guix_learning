...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_display_driver_generic_ellipse_fill                             */
/*                                                           6.1.6        */
/*  AUTHOR                                                                */
/*                                                                        */
...
INT                   realval;
    ...
    if (pixelmap != GX_NULL)
    {
        skip_line = (skip_line % info.pixelmap->height);
        if (skip_line)
        {
            info.draw = GX_FALSE;
            while (skip_line--)
        ...
        for (y = ymin; y <= ymax; y++)
        {
            ...
            display->driver_horizontal_pixelmap_line_draw(context, pLineEnds[Index], pLineEnds[Index + 1], y, &info);
            Index += 2;
        }
    }
    else
    {
        ...
        for (y = ymin; y <= ymax; y++)
        {
            display->driver_horizontal_line_draw(context, pLineEnds[Index], pLineEnds[Index + 1], y, 1, fill_color);
            Index += 2;
        }
    }
}
