...
/*                                                                        */
/**************************************************************************/
static void _gx_utility_glyph_8bpp_to_alphamap_draw(GX_PIXELMAP *map, INT xpos, INT ypos, const GX_GLYPH *glyph)
{
GX_UBYTE *read_data;
GX_UBYTE *read_row;
GX_UBYTE *write_data;
...
USHORT    write_stride;
    ...
    if (map->flags & (GX_PIXELMAP_ROTATED_90 | GX_PIXELMAP_ROTATED_270))
    {
        write_stride = (USHORT)map->height;
        pixel_width = glyph->height;
        y_height = glyph->width;
        GX_SWAP_VALS(xpos, ypos)

        if (map->flags & GX_PIXELMAP_ROTATED_90)
        {
            ypos = (map->width - ypos - glyph->width);
        }
        else
        {
            xpos = (map->height - xpos - glyph->height);
        }
    }
    else
    {
        write_stride = (USHORT)map->width;
        pixel_width = glyph->width;
        y_height = glyph->height;
    }

    read_row = (GX_UBYTE *)glyph->map;

    write_row = (GX_UBYTE *)map->data;
...
/*                                                                        */
/**************************************************************************/
static void _gx_utility_glyph_4bpp_to_alphamap_draw(GX_PIXELMAP *map, INT xpos, INT ypos, const GX_GLYPH *glyph)
{
GX_UBYTE *read_data;
GX_UBYTE *read_row;
GX_UBYTE *write_data;
...
GX_UBYTE  data;
    ...
    if (map->flags & (GX_PIXELMAP_ROTATED_90 | GX_PIXELMAP_ROTATED_270))
    {
        ...
        if (map->flags & GX_PIXELMAP_ROTATED_90)
        {
            ypos = (INT)map->width - (INT)ypos - (INT)y_height;
        }
        else
        {
            xpos = (INT)map->height - (INT)xpos - (INT)pixel_width;
        }
    }
    else
    {
        pixel_width = (USHORT)glyph->width;
        y_height = glyph->height;
        write_stride = (USHORT)map->width;
    }
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_utility_glyph_reversed_4bpp_to_alphamap_draw                    */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
/**************************************************************************/
#if defined(GX_RENESAS_DAVE2D_FONT_SUPPORT)
static void _gx_utility_glyph_reversed_4bpp_to_alphamap_draw(GX_PIXELMAP *map, INT xpos, INT ypos, const GX_GLYPH *glyph)
{
GX_UBYTE *read_data;
GX_UBYTE *read_row;
GX_UBYTE *write_data;
...
/*                                                                        */
/**************************************************************************/
static void _gx_utility_glyph_1bpp_to_alphamap_draw(GX_PIXELMAP *map, INT xpos, INT ypos, const GX_GLYPH *glyph)
{
GX_UBYTE *read_data;
GX_UBYTE *read_row;
GX_UBYTE *write_data;
...
GX_UBYTE  mask;
    ...
    if (map->flags & (GX_PIXELMAP_ROTATED_90 | GX_PIXELMAP_ROTATED_270))
    {
        write_stride = (USHORT)map->height;
        y_height = glyph->width;
        glyph_width = (USHORT)glyph->height;
        GX_SWAP_VALS(xpos, ypos)
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_utility_glyph_reversed_1bpp_to_alphamap_draw                    */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
/**************************************************************************/
#if defined(GX_RENESAS_DAVE2D_FONT_SUPPORT)
static void _gx_utility_glyph_reversed_1bpp_to_alphamap_draw(GX_PIXELMAP *map, INT xpos, INT ypos, const GX_GLYPH *glyph)
{
GX_UBYTE *read_data;
GX_UBYTE *read_row;
GX_UBYTE *write_data;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_utility_string_to_alphamap_draw                                 */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
/*                                                                        */
/**************************************************************************/
void _gx_utility_string_to_alphamap_draw(const GX_STRING *string, const GX_FONT *font, GX_PIXELMAP *map)
{
INT                xpos;
int                y_offset;
...
void               (*glyph_draw)(GX_PIXELMAP *map, INT xpos, INT ypos, const GX_GLYPH *glyph);
...
UINT glyph_len;
    ...
    if (font->format & GX_FONT_FORMAT_ROTATED_90)
    {
        map->flags |= GX_PIXELMAP_ROTATED_90;
    }
    else if (font->format & GX_FONT_FORMAT_ROTATED_270)
    {
        map->flags |= GX_PIXELMAP_ROTATED_270;
    }
...
#if defined(GX_RENESAS_DAVE2D_FONT_SUPPORT)
        if (font->format & GX_FONT_FORMAT_REVERSED_ORDER)
        {
    ...
    while (string_copy.length)
    {
        ...
        while (font_page)
        {
            if (font_page->first_glyph <= char_val &&
                font_page->last_glyph >= char_val)
            {
                break;
            }
            font_page = font_page->next_page;
        }
        if (font_page)
        {
            glyph_index = (GX_CHAR_CODE)(char_val - font_page->first_glyph);
            glyph = &font_page->glyphs.normal[glyph_index];
            y_offset = font_page->baseline - glyph->ascent;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_utility_string_to_alphamap_ext                                  */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
...
/*                                                                        */
/**************************************************************************/
UINT _gx_utility_string_to_alphamap_ext(const GX_STRING *string, const GX_FONT *font, GX_PIXELMAP *textmap)
{
GX_VALUE           alphamap_width;
GX_VALUE           alphamap_height;
UINT               status;
const GX_FONT  *font_page = font;
GX_CHAR_CODE       char_val;
const GX_GLYPH *glyph;
...
UINT glyph_len;
    ...
    while (string_copy.length)
    {
        ...
        if (first_glyph || string_copy.ptr[0] == GX_NULL)
        {
            while (font_page)
            {
                if (font_page->first_glyph <= char_val &&
            ...
            if (font_page)
            {
                glyph = &font_page->glyphs.normal[char_val - font_page->first_glyph];
                ...
                if (first_glyph)
                {
                    ...
                    if (glyph->leading < 0)
                    {
                        alphamap_width = (GX_VALUE)(alphamap_width - glyph->leading);
                    }
                }
                else
                {
                    ...
                    {
                        alphamap_width = (GX_VALUE)(alphamap_width + glyph->width - glyph->advance);
                    }
                }
            }
        }
    ...
    alphamap_height = font->line_height;
