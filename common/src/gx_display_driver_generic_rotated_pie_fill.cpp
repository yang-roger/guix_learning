...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_display_driver_generic_rotated_simple_pie_fill                  */
/*                                                           6.1.5        */
/*  AUTHOR                                                                */
/*                                                                        */
...
GX_FILL_PIXELMAP_INFO info;
    ...
    brush = &context->brush;
    line_draw = display->driver_vertical_line_draw;
    ...
    while (curx <= cury)
    {
        for (loop = 0; loop < 4; loop++)
        {
            ...
            if ((point.x >= xmin) && (point.x <= xmax))
            {
                ...
                {
                    Index = (point.x - xmin) << 1;
                    if (point.y < pLineEnds[Index])
                    {
                        pLineEnds[Index] = point.y;
                    }

                    if (point.y > pLineEnds[Index + 1])
                    {
                        pLineEnds[Index + 1] = point.y;
                    }
                }
            }
    ...
    if (pixelmap)
    {
        ...
        {
            skip_line = (xmax - (xcenter - (INT)r) + 1) % pixelmap->width;
        ...
        {
            if (pLineEnds[Index] < clip->top)
            {
                pLineEnds[Index] = clip->top;
            }

            if (pLineEnds[Index + 1] > clip->bottom)
            {
                pLineEnds[Index + 1] = clip->bottom;
            }

            info.x_offset = pLineEnds[Index] - xpos;

            /* Filling pie area with pixelmap. */
            display->driver_horizontal_pixelmap_line_draw(context, pLineEnds[Index], pLineEnds[Index + 1], xstart, &info);
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_display_driver_generic_rotated_pie_fill                         */
/*                                                           6.1.3        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Kenneth Maxwell, Microsoft Corporation                              */
