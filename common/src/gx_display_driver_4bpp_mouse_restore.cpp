...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _gx_display_driver_4bpp_mouse_restore                               */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
    ...
    if (display->mouse.cursor_info)
    {
            ...
            (display->mouse.status & GX_MOUSE_VISIBLE))
        {
            ...
            if (width > 0 && height > 0)
            {
                ...
                putrow += ((canvas->x_resolution + 1) >> 1) * display->mouse.rect.top;
                putrow += display->mouse.rect.left >> 1;

                for (row = 0; row < height; row++)
                {
                    getmask = 0xf0;
                    if (display->mouse.rect.left & 1)
                    {
                        putmask = 0x0f;
                    }
                    else
