...
GX_WIDGET *frame;
    ...
    // fully opaque
    transition_animation_info.end_alpha = 255;
    transition_animation_info.start_alpha = 255;

    // end position inside child frame:
    transition_animation_info.end_position.x = frame->size.left + 1;
    transition_animation_info.end_position.y = frame->size.top + 1;

    // calculate start position:
    transition_animation_info.start_position = transition_animation_info.end_position;
    ...
    case SLIDE_DOWN:
        ...
        {
            transition_animation_info.start_position.y -= offset;
        }
        else
        {
            transition_animation_info.end_position.y += offset;
        }
        break;
    ...
    case SLIDE_LEFT:
        ...
        if (animation_id == ANIMATION_SETTINGS_SLIDE_IN)
        {
            transition_animation_info.start_position.x += offset;
        }
        else
        {
            transition_animation_info.end_position.x -= offset;
        }
        break;
    ...
    transition_animation_info.frame_interval = 1;
    transition_animation_info.steps = 10;
    transition_animation_info.parent = frame;
    transition_animation_info.target = (GX_WIDGET *) &settings_win;
    transition_animation_info.style = GX_ANIMATION_TRANSLATE | GX_ANIMATION_SINE_EASE_IN;
    ...
    {
        transition_animation_info.style |= GX_ANIMATION_DETACH;
    ...
    transition_animation_info.id = animation_id;
...
UINT BackgroundWinEventProcess(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_SHOW:
        ...
        if (!drag_animation)
        {
            ...
            drag_animation->landing_speed = 10;

            memset(&drag_animation_info, 0, sizeof(GX_ANIMATION_INFO));
            drag_animation_info.parent = (GX_WIDGET *) &background_win.background_win_child_frame;
            drag_animation_info.style = GX_ANIMATION_SCREEN_DRAG | GX_ANIMATION_WRAP | GX_ANIMATION_HORIZONTAL;
            drag_animation_info.frame_interval = 1;
            drag_animation_info.slide_screen_list = drag_screen_list;
    ...
    case GX_SIGNAL(IDB_SETTINGS, GX_EVENT_CLICKED):
        if (!(settings_win.status & GX_STATUS_VISIBLE))
        {
