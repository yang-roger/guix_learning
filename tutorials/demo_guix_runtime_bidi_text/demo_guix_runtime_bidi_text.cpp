...
UINT main_screen_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_BUTTON_ENGLISH, GX_EVENT_RADIO_SELECT):
        /* Switch to English. */
        gx_display_active_language_set(root->canvas->display, LANGUAGE_ENGLISH);
    ...
    case GX_SIGNAL(ID_BUTTON_ARABIC, GX_EVENT_RADIO_SELECT):
        /* Switch to Arabic. */
        gx_display_active_language_set(root->canvas->display, LANGUAGE_ARABIC);
...
VOID window_draw(GX_WINDOW *window)
{
    ...
    /* Set bidi text. */
    gx_context_string_get_ext(GX_STRING_ID_WINDOW_TEXT, &info.text);

    /* Set text draw font. */
    gx_context_font_get(GX_FONT_ID_ARABIC, &info.font);

    /* Set text display width for line breaking, if there is no need for line breaking, set display width to -1. */
    gx_window_client_width_get(window, &info.display_width);

    xpos = window->client.left;
    ypos = window->client.top;
    ...
    if (gx_utility_bidi_paragraph_reorder(&info, &resolved_head) == GX_SUCCESS)
    {
        ...
        do
        {
            for (line = 0; line < resolved->total_lines; line++)
            {
                gx_canvas_text_draw_ext(xpos, ypos, &resolved->text[line]);

                ypos += info.font->line_height;
            }
