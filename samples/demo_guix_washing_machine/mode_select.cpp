...
VOID mode_select_window_init()
{
    ...
    while (entry->button)
    {
        ...
        if (selected_button_id != entry->button->id)
        {
            /* Hide big button.  */
...
MODE_BTN_INFO *get_mode_button_info(INT button_id)
{
    ...
    while (entry->button)
    {
        if ((button_id == entry->button->id) ||
            (button_id == entry->button_small->id))
        {
            return entry;
        }
...
VOID button_select(INT button_id)
{
    ...
    if (info)
    {
        ...
        selected_button_id = info->button->id;
    }
}
...
UINT mode_select_window_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_BTN_DEEP_SMALL, GX_EVENT_CLICKED):
        if (selected_button_id == event_ptr->sender)
        {
            set_cycle_mode();
        }
        else
        {
            button_select(event_ptr->sender);
        }
        break;
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == ID_TIMER_PROGRESS_BAR)
        {
