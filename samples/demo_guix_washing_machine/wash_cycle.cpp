...
{
    INT index;
    GX_RADIAL_SLIDER_INFO* info = &wash_cycle_window.wash_cycle_window_wash_cycle_slider.info;
...
static VOID update_progress_bar_value(INT angle)
{
    ...
    INT percent = (info->max_angle - angle) * 100 /
                  (info->max_angle - info->min_angle);
...
UINT wash_cycle_window_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_WASH_CYCLE_SLIDER, GX_EVENT_SLIDER_VALUE):
        update_progress_bar_value(event_ptr->payload.longdata);
        break;
...
VOID wash_cycle_slider_draw(GX_RADIAL_SLIDER* slider)
{
    ...
    while (arc->start_angle < arc->end_angle)
    {
        ...
        {
            gx_canvas_arc_draw(xpos, ypos, info->radius, start_angle, arc->end_angle);
        }
