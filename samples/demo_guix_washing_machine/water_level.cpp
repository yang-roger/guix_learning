...
VOID start_water_level_slider_animation()
{
    ...
    water_level = info->current_angle;
    gx_radial_slider_angle_set(slider, info->min_angle);
...
{
    GX_RADIAL_SLIDER_INFO *info = &water_level_window.water_level_window_water_level_slider.info;
    INT water_level;

    /* Convert angle value to temperature value.  */
    water_level = (info->current_angle - info->min_angle) *
...
UINT water_level_window_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_WATER_LEVEL_SLIDER, GX_EVENT_SLIDER_VALUE):
        update_water_level_numeric_values(event_ptr->payload.longdata);
        break;
    ...
    case GX_SIGNAL(ID_BTN_MIN, GX_EVENT_RADIO_SELECT):
        update_water_level_slider_value(event_ptr->sender);
        break;
