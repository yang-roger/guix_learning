...
/******************************************************************************************/
VOID repeat_selection_style_button_select(GX_BUTTON *button)
{
    button->style &= ~GX_STYLE_BUTTON_PUSHED;
...
VOID  guix_setup()
{
    ...
    main_screen.main_screen_btn_wash_cycle.select_handler = repeat_selection_style_button_select;
...
VOID mark_wave_animation_parent_dirty()
{
    ...
    if (wash_cycle_window.status & GX_STATUS_VISIBLE)
    {
        parent = (GX_WIDGET *)&wash_cycle_window;
    }
    else if (temperature_window.status & GX_STATUS_VISIBLE)
    {
    ...
    /* Calculate wave area. */
    dirty_area = parent->size;
    dirty_area.left = (GX_VALUE)(dirty_area.left + WASH_CYCLE_TEMPERATURE_RING_WIDTH);
    dirty_area.top = (GX_VALUE)(dirty_area.top + WASH_CYCLE_TEMPERATURE_RING_WIDTH);
    dirty_area.right = (GX_VALUE)(dirty_area.right - WASH_CYCLE_TEMPERATURE_RING_WIDTH);
    dirty_area.bottom = (GX_VALUE)(dirty_area.bottom - WASH_CYCLE_TEMPERATURE_RING_WIDTH);
...
UINT main_screen_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_SIGNAL(ID_BTN_WASH_CYCLE, GX_EVENT_RADIO_SELECT):
        ...
        {
            if (mode_select_window.status & GX_STATUS_VISIBLE)
            {
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == ID_TIMER_WAVE_ANIMATION)
        {
            wave_rotation_angle = (358 + wave_rotation_angle) % 360;

            mark_wave_animation_parent_dirty();
        }
        if(event_ptr->payload.timer_id == ID_TIMER_PROGRESS_BAR_ANIMATION)
        {
            gx_system_dirty_mark(&main_screen.main_screen_wash_cycle_progress_bar);
        }
        if(event_ptr->payload.timer_id == ID_TIMER_TIME)
        {
...
{
    if (prompt->parent->style & GX_STYLE_BUTTON_PUSHED)
    {
        prompt->style |= GX_STYLE_DRAW_SELECTED;
    }
    else
    {
        prompt->style &= (~GX_STYLE_DRAW_SELECTED);
...
VOID time_format(GX_NUMERIC_PROMPT* prompt, INT value)
{
    ...
    {
        prompt->buffer[0] = '0';
        prompt->buffer[1] = '0' + value;
    }
    else if (value < 100)
    {
        prompt->buffer[0] = '0' + (value / 10);
        prompt->buffer[1] = '0' + (value % 10);
    }
}
...
VOID animation_wave_draw(GX_WINDOW* window)
{
    ...
    width = size->right - size->left + 1;
    height = size->bottom - size->top + 1;
    xcenter = size->left + (width >> 1);
    ycenter = size->top + (height >> 1);
...
VOID wash_cycle_progress_bar_draw(GX_PROGRESS_BAR* progress_bar)
{
    ...
    INT ypos;
    INT selected_dist;
    GX_RECTANGLE *size = &progress_bar->size;
    GX_BRUSH *brush;
    INT alpha_shift;
    ...
    brush->alpha = 100;
    for (xpos = size->left; xpos < size->right; xpos += PROGRESS_DOT_SPACE)
    {
    ...
    selected_dist = (size->right - size->left) * progress_bar->info.current_val / 100;

    /* Draw selected dots.  */
    if (main_screen.main_screen_btn_play.style & GX_STYLE_BUTTON_PUSHED)
    {
        ...
        brush->alpha = progress_alpha_start;
        for (xpos = size->left; xpos < size->left + selected_dist; xpos += PROGRESS_DOT_SPACE)
        {
            gx_canvas_pixelmap_draw(xpos, ypos, map);
            if (brush->alpha + alpha_shift < 255)
            {
                brush->alpha += alpha_shift;
            }
            else
            {
                brush->alpha = 0;
