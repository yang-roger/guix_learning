...
/* Extern message tmpl definition.  */
GX_EXTERN_C GX_STUDIO_WIDGET message_template_define;
...
{
    GX_WIDGET *child;
    INT left = parent->size.left;
    INT top = parent->size.top;

    child = parent->first_child;
    while (child)
    {
        if (!(child->status & GX_STATUS_NONCLIENT))
        {
            
            gx_widget_shift(child, left - child->size.left, top - child->size.top, GX_FALSE);
            top = child->size.bottom + 1;
        }
...
static INT calculate_text_height(GX_STRING *string, GX_VALUE client_width, GX_RESOURCE_ID font_id)
{
    ...
    {
        tmp_buffer[0] = string->ptr[index];
        
        gx_system_string_width_get_ext(font, &tmp, &tmp_width);
        text_width += tmp_width;

        if ((tmp_buffer[0] == '\r') || (index == string->length - 1))
        {
            text_height += font->line_height * (1 + text_width / client_width);
            text_width = 0;
        }
    }
...
static VOID screen_meesage_update()
{
    ...
    for (index = 0; index < MSG_COUNT; index++)
    {
        ...
        text_height = calculate_text_height(&info->msg, width, view->font_id_());
        size = view->size;
        size.bottom = size.top + text_height + (view->margin << 1);
