...
/* Update watch hands.                                                                    */
/******************************************************************************************/
static VOID clock_gauge_update(CLOCK_SCREEN_TEMPLATE_CONTROL_BLOCK* tmpl)
{
    INT hour_angle;
    INT minute_angle;
    ...
    gx_circular_gauge_angle_set(&tmpl->clock_screen_template_gauge, system_time.second * 6);

    if (mark_dirty)
    {
        gx_system_dirty_mark(&tmpl->clock_screen_template_gauge);
    }
}
...
static VOID draw_needle(GX_PIXELMAP *map, INT angle, INT xcenter, INT ycenter)
{
    ...
    if (gx_utility_pixelmap_rotate(map, angle, &needle_rotated, &xcor, &ycor) == GX_SUCCESS)
    {
        ...
        if (needle_rotated.aux_data)
        {
            memory_free((VOID*)needle_rotated.aux_data);
        }
        memory_free((VOID*)needle_rotated.data);
    }
    else
    {
        /* Draw needle .  */
...
static VOID clock_gauge_draw(GX_CIRCULAR_GAUGE *gauge, GX_RESOURCE_ID hands_color_id, GX_RESOURCE_ID hands_center_map_id)
{
    ...
    xcenter = gauge->size.left + info->needle_xpos;
    ycenter = gauge->size.top + info->needle_ypos;
    ...
    if (map)
    {
        gx_canvas_pixelmap_draw((GX_VALUE)(xcenter - (map->width >> 1)),
            (GX_VALUE)(ycenter - (map->height >> 1)), map);
    }
}
...
{
    gx_widget_fill_color_set(&clock_1_screen, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE);
    gx_widget_fill_color_set(&clock_2_screen, GX_COLOR_ID_CANVAS, GX_COLOR_ID_CANVAS, GX_COLOR_ID_CANVAS);

    gx_icon_pixelmap_set((GX_ICON*)&clock_1_screen.clock_screen_template_gauge, GX_PIXELMAP_ID_W_DIAL_BLACK, GX_PIXELMAP_ID_W_DIAL_BLACK);
    gx_icon_pixelmap_set((GX_ICON*)&clock_2_screen.clock_screen_template_gauge, GX_PIXELMAP_ID_W_DIAL_WHITE, GX_PIXELMAP_ID_W_DIAL_WHITE);

    gx_widget_draw_set((GX_ICON*)&clock_1_screen.clock_screen_template_gauge, clock_1_gauge_draw);
    gx_widget_draw_set((GX_ICON*)&clock_2_screen.clock_screen_template_gauge, clock_2_gauge_draw);
}
