...
static VOID screen_time_update()
{
    ...
    if (home_screen.home_screen_am_pm.text_id_() != am_pm_text_id)
    {
    ...
    if ((screen_time_record.month != system_time.month) || (screen_time_record.day != system_time.day))
    {
        ...
        if (string.ptr)
        {
            sprintf(date_string_buffer, ", %s %d", string.ptr, system_time.day);

            string.ptr = date_string_buffer;
            string.length = string_length_get(date_string_buffer, sizeof(date_string_buffer) - 1);
