...
VOID fitness_screen_initialize()
{
    ...
    ekg_map.data = (GX_UBYTE*)ekg_map_data_buffer;
    ekg_map.data_size = sizeof(ekg_map_data_buffer);
    ekg_map.height = EKG_MAP_HEIGHT;
    ekg_map.width = EKG_MAP_WIDTH;
    ekg_map.format = GX_COLOR_FORMAT_24XRGB;

    memset(&ekg_canvas, 0, sizeof(GX_CANVAS));

    /* Create a canvas for drawing ekg. */
    gx_canvas_create(&ekg_canvas, "ekg",
        root->canvas->display, GX_CANVAS_SIMPLE,
        EKG_MAP_WIDTH, EKG_MAP_HEIGHT, (GX_COLOR*)ekg_map.data, ekg_map.data_size);
...
VOID ekg_waveform_win_draw(GX_WINDOW* window)
{
    ...
    /* Draw ekg waveform.  */
    gx_canvas_pixelmap_draw(size->left, size->top, &ekg_map);
    ...
    gx_canvas_pixelmap_draw(size->right - line_cover_gradient->width + 1, size->top, line_cover_gradient);
}