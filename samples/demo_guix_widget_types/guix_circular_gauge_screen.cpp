...
void set_speed_value(int id, int value)
{
    ...
    if (id == 1)
    {
        ...
        string.ptr = prompt_text_animated;
    }
    else
    {
        prompt = &(&Gauge_Screen)->Gauge_Screen_prompt_not_animated;
        string.ptr = prompt_text_not_animated;
    }

    gx_utility_ltoa(value, (GX_CHAR *)string.ptr, MAX_TEXT_LENGTH + 1);

    string.length = string_length_get(string.ptr, MAX_TEXT_LENGTH);
