...
/*****************************************************************************/
static VOID *rotate_memory_allocate(ULONG size)
{
    VOID *memptr;
...
/*****************************************************************************/
static VOID rotate_memory_free(VOID *mem)
{
    tx_byte_release(mem);
}
...
UINT text_screen_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == PROMPT_VALUE_UPDATE_TIMER)
        {
...
void slider_value_update(GX_EVENT *event_ptr)
{
    ...
    string.ptr = slider_val_text;

    pos = event_ptr->payload.longdata;
    gx_progress_bar_value_set(&Slider_Screen.Slider_Screen_Progress_Bar, pos);
    gx_utility_ltoa(pos, (GX_CHAR *)string.ptr, MAX_SLIDER_VAL_TEXT_LENGTH);
    strncat((GX_CHAR *)string.ptr, "%", 1);
    string.length = string_length_get(string.ptr, MAX_SLIDER_VAL_TEXT_LENGTH);
...
UINT slider_screen_event_process(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == SLIDER_ANIMATION_TIMER)
        {
...
VOID reset_sliders(GX_WINDOW *parent)
{
    ...
    slider_index = 0;
    widget = parent->first_child;

    while(widget)
    {
        if (widget->type == GX_TYPE_SLIDER ||
            widget->type == GX_TYPE_PIXELMAP_SLIDER)
        {
            slider = (GX_SLIDER *) widget;
            slider_vals[slider_index].slider = slider;
            slider_vals[slider_index].target_val = slider->info.current_val << 4;
            ...
            gx_slider_value_set(slider, &slider->info, 0);
            slider_index++;
        }
        widget= widget->next;
    }
...
GX_BOOL update_sliders()
{
    ...
    while(sval->slider)
    {
        if (sval->current_val != sval->target_val)
        {
            ...
            gx_slider_value_set(sval->slider, &sval->slider->info, value >> 4);
        }
        sval++;
    }
    return status;
...
VOID drop_list_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    string.length = string_length_get(entry->text, MAX_ROW_TEXT_LENGTH);
...
VOID PopulateDropList(void)
{
    ...
    sa.thumb_pixelmap = GX_PIXELMAP_ID_LIST_SCROLL_THUMB;
    sa.thumb_travel_min = 4;
    sa.thumb_travel_max = 4;
    sa.thumb_width = 8;
    sa.width = 10;
...
VOID custom_icon_button_draw(GX_ICON_BUTTON *button)
{
    ...
    // draw the background
    if (button->style & GX_STYLE_BUTTON_PUSHED)
    {
    ...
    gx_canvas_pixelmap_draw(button->size.left,
                            button->size.top,
                            map);
    ...
    xpos = button->size.right - button->size.left + 1;
    xpos -= map->width;
    xpos /= 2;
    xpos += button->size.left + icon_offset;

    ypos = button->size.bottom - button->size.top + 1;
    ypos -= map->height;
    ypos /= 2;
    ypos += button->size.top + icon_offset;
...
VOID custom_transparent_prompt_thick_border_draw(GX_PROMPT *prompt)
{
    ...
    gx_canvas_pixelmap_draw(prompt->size.left,
        prompt->size.top,
        map);
...
GX_PIXELMAP *map;
    ...
    gx_canvas_pixelmap_draw(frame->size.left,
                            frame->size.top, map);

    gx_context_pixelmap_get(GX_PIXELMAP_ID_UR_CORNER, &map);
    gx_canvas_pixelmap_draw(frame->size.right - map->width + 1,
                            frame->size.top, map);

    gx_context_pixelmap_get(GX_PIXELMAP_ID_LL_CORNER, &map);
    gx_canvas_pixelmap_draw(frame->size.left,
                            frame->size.bottom - map->height + 1, map);
...
VOID custom_next_button_draw(GX_PIXELMAP_BUTTON *pixelmap_button)
{
    ...
    x_pos = pixelmap_button->size.left;
    y_pos = pixelmap_button->size.top;

    if (pixelmap_button->style & GX_STYLE_BUTTON_PUSHED)
    {
        x_pos += 2;
        y_pos += 2;
    }
...
{
    gx_utility_ltoa(value % 10, prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE);
}
...
{
    int length;
    gx_utility_ltoa(value / 100, prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE);

    length = string_length_get(prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE);
    prompt->buffer[length++] = '.';
    gx_utility_ltoa(value % 100, prompt->buffer + length, GX_NUMERIC_PROMPT_BUFFER_SIZE - length);
}
