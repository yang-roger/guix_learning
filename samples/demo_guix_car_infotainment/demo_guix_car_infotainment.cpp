...
{
    INT index;
    GX_RADIAL_SLIDER_INFO* info = &main_screen.main_screen_radial_slider.info;

    arc_info_list[0].end_angle = info->max_angle;
...
static VOID speedometer_aniomation_update()
{
    ...
    angle = MPH_values[MPH_value_index] * (info->max_angle - info->min_angle) / MAX_MPH;
    angle = info->max_angle - angle;
...
VOID time_update()
{
    ...
    {
        if (main_screen.main_screen_weather_win.style & GX_STYLE_ENABLED)
        {
...
VOID time_value_format(GX_NUMERIC_PROMPT *prompt, INT value)
{
    ...
    if (value < 10)
    {
        ...
        prompt->buffer[index++] = '0' + value;
    }
    else
    {
        prompt->buffer[index++] = '0' + (value / 10);
        prompt->buffer[index++] = '0' + (value % 10);
    }
...
{
    GX_WIDGET* child = button->first_child;
    GX_BRUSH* brush;

    gx_context_brush_get(&brush);

    if (button->id != ID_ON_OFF_BTN)
    {
        brush->alpha = bottom_button_alpha;
    }
...
{
    GX_RADIAL_SLIDER_INFO* info = &slider->info;
    GX_PIXELMAP* map;
    INT xpos;
    INT ypos;
    ...
    gx_context_pixelmap_get(info->background_pixelmap, &map);

    xpos = slider->size.left;
    ypos = slider->size.top;
    ...
    radius = info->radius;
    radius += (info->track_width / 2);
    radius -= 2;

    /* Fill outer ring with disconnected arcs. */
    while (arc->start_angle < arc->end_angle)
    {
        if (info->current_angle < arc->start_angle)
        {
            start_angle = arc->start_angle;
        }
        else
        {
            start_angle = info->current_angle;
        }
        ...
        if (start_angle == info->current_angle)
        {
            break;
        }
    ...
    gx_utility_circle_point_get(xpos,
        ypos,
        info->radius - (info->track_width >> 1) + 1,
        angle,
        &inner_pt);
    ...
    /* Draw needle. */
    gx_canvas_line_draw(inner_pt.x, inner_pt.y, outer_pt.x, outer_pt.y);
