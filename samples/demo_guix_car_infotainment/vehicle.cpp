...
GX_SPRITE *sprite;
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == TIMER_ID_VEHICLE_NUMBER_ANIMATION)
        {
            /* Update number animation. */
    ...
    case GX_SIGNAL(ID_LIGHT_SPRITE_RIGHT, GX_EVENT_SPRITE_COMPLETE):
        gx_widget_find(window, event_ptr->sender, 2, &sprite);

        /* Toggle light frame list. */
        if (sprite->frame_list[0].pixelmap == light_frame_list_1[0].pixelmap)
        {
