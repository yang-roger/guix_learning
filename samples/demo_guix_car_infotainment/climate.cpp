...
INT progress_val_to_seat_angle(GX_PROGRESS_BAR* bar)
{
    ...
    seat_angle = info->current_val - info->min_val;
    seat_angle *= (MAX_SEAT_ANGLE - MIN_SEAT_ANGLE);
    seat_angle /= info->max_val - info->min_val;
...
VOID seat_angle_animation_start()
{
    ...
    target_left_progress_val = climate_screen.climate_screen_left_progress_bar.info.current_val;
    target_right_progress_val = climate_screen.climate_screen_right_progress_bar.info.current_val;
...
UINT climate_screen_event_process(GX_WINDOW *window, GX_EVENT* event_ptr)
{
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == TIMER_ID_CLIMATE_SCREEN_PROGRESS_ANIMATION)
        {
