...
VOID music_play_update()
{
    ...
    GX_VALUE val = bar->info.current_val;
...
VOID slide_in_music_info_win()
{
    ...
    my_event.target = (GX_WIDGET*)&audio_screen;
    my_event.type = USER_EVENT_SLIDE_IN_MUSIC_INFO_WIN;
...
UINT audio_screen_event_process(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == TIMER_ID_RADIAL_PROGRESS_ANIMATION)
        {
            /* Update music play progress.  */
            music_play_update();
        }
