...
{
    GX_WIDGET *child = main_screen.main_screen_btn_window.first_child;

    while (child)
    {
        if ((child->type == GX_TYPE_PIXELMAP_BUTTON) &&
            (child->style & GX_STYLE_BUTTON_PUSHED))
        {
            ((GX_BUTTON *)child)->deselect_handler((GX_BUTTON *)child, GX_FALSE);
...
{
    GX_WIDGET* child = parent->first_child;
...
UINT base_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    switch (event_ptr->type)
    {
        ...
        case GX_SIGNAL(ID_ON_OFF_BTN, GX_EVENT_CLICKED):
            if (main_screen.main_screen_on_off_text.text_id_() == GX_STRING_ID_OFF)
            {
