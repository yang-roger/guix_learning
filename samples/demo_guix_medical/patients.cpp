...
static VOID patient_background_draw(GX_WIDGET* widget)
{
    ...
    gx_canvas_line_draw(widget->size.left, widget->size.bottom,
                        widget->size.right, widget->size.bottom);
}
...
static VOID patient_prompt_draw(GX_PROMPT* prompt)
{
    ...
    if (prompt->parent)
    {
        if (prompt->parent->style & GX_STYLE_DRAW_SELECTED)
        {
...
GX_STRING    string;
    ...
    if (!result)
    {
        ...
        gx_utility_rectangle_define(&childsize, list->client.left, 0,
                                                list->client.right, PATIENT_LIST_ROW_HEIGHT);
        ...
        childsize.left = patients_screen.patients_screen_patient_name_label.size.left;
        childsize.right = patients_screen.patients_screen_DOB_label.size.left - 1;
        ...
        childsize.left = childsize.right + 1;
        childsize.right = patients_screen.patients_screen_room_label.size.left - 1;
        ...
        childsize.left = childsize.right + 1;
        childsize.right = patients_screen.patients_screen_admission_date_label.size.left - 1;
        ...
        childsize.left = childsize.right + 1;
        childsize.right = patients_screen.patients_screen_pa_num_label.size.left - 1;
        ...
        childsize.left = childsize.right + 1;
        childsize.right = list->client.right;
    ...
    /* Update patinet name.  */
    string.ptr = patient_list[index].name;
    string.length = string_length_get(string.ptr, MAX_STRING_LENGTH);
    gx_prompt_text_set_ext(&row->name, &string);

    /* Update date of birth.  */
    string.ptr = patient_list[index].dob;
    string.length = string_length_get(string.ptr, MAX_STRING_LENGTH);
    gx_prompt_text_set_ext(&row->dob, &string);

    /* Update rom number.  */
    string.ptr = patient_list[index].room_num;
    string.length = string_length_get(string.ptr, MAX_STRING_LENGTH);
    gx_prompt_text_set_ext(&row->room_num, &string);

    /* Update admission date.  */
    string.ptr = patient_list[index].admit_date;
    string.length = string_length_get(string.ptr, MAX_STRING_LENGTH);
    gx_prompt_text_set_ext(&row->admit_date, &string);

    /* Update mr number.  */
    string.ptr = patient_list[index].mr_num;
    string.length = string_length_get(string.ptr, MAX_STRING_LENGTH);
...
VOID GetPatientName(GX_STRING* string)
{
    string->ptr = patient_list[selected_patient_index].name;
    string->length = string_length_get(string->ptr, MAX_STRING_LENGTH);
}
...
VOID GetPatientDOB(GX_STRING* string)
{
    string->ptr = patient_list[selected_patient_index].dob;
    string->length = string_length_get(string->ptr, MAX_STRING_LENGTH);
}
