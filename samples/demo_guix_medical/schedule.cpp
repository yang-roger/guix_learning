...
{
    if (schedule_screen.schedule_screen_active_bar.parent == GX_NULL)
    {
        if (schedule_screen.schedule_screen_active_circle.parent)
        {
...
static VOID update_time_colon_animation()
{
    ...
    if (schedule_screen.schedule_screen_colon.disabled_text_color_() == GX_COLOR_ID_GRAY)
    {
...
UINT schedule_screen_event_handler(GX_WINDOW* window, GX_EVENT* myevent)
{
    ...
    case GX_EVENT_ANIMATION_COMPLETE:
        if (myevent->sender == ID_R5_WIN_FADE_IN)
        {
...
INT           index;
    ...
    xoffset = (schedule_screen.schedule_screen_c2_day.size.left - 
               schedule_screen.schedule_screen_c1_day.size.left);

    yoffset = (schedule_screen.schedule_screen_r2_hour.size.top -
               schedule_screen.schedule_screen_r1_hour.size.top);

    xpos = schedule_screen.schedule_screen_r1_am_pm.size.right + 5;
    ypos = schedule_screen.schedule_screen_r1_am_pm.size.bottom - 5;
    ...
    {
        gx_canvas_line_draw(xpos, ypos, client->right, ypos);

        ypos += yoffset;
    }

    xpos = schedule_screen.schedule_screen_c1_week.size.left - 10;
    ypos -= yoffset;
    ...
    for (index = 0; index < SCHEDULE_COL_COUNT; index++)
    {
        ...
        gx_canvas_line_draw(xpos, client->top, xpos, ypos);
    ...
    size = &schedule_screen.schedule_screen_current_time.size;

    xpos = schedule_screen.schedule_screen_r1_am_pm.size.right + 5;
    ypos = (size->top + size->bottom) >> 1;
    gx_canvas_line_draw(xpos, ypos, size->left - 1, ypos);
