...
/******************************************************************************************/
/* Override the default event processing of "win" to handle signals from my child    */
/* widgets.                                                                               */
/******************************************************************************************/
UINT template_event_handler(GX_WINDOW* window, GX_EVENT* event_ptr)
{
UINT                    status = GX_SUCCESS;
TEMPLATE_CONTROL_BLOCK *win = (TEMPLATE_CONTROL_BLOCK *)window;
    ...
    case GX_EVENT_SHOW:
        ...
        gx_widget_style_remove(&win->template_vitals_button, GX_STYLE_BUTTON_PUSHED);
        gx_widget_style_remove(&win->template_schedule_button, GX_STYLE_BUTTON_PUSHED);
        gx_widget_style_remove(&win->template_patients_button, GX_STYLE_BUTTON_PUSHED);

        /* Selected navigation button according to the active screen.  */
        switch (window->id)
        {
        case ID_VITALS_SCREEN:
            gx_widget_style_add(&win->template_vitals_button, GX_STYLE_BUTTON_PUSHED);
            break;

        case ID_SCHEDULE_SCREEN:
            gx_widget_style_add(&win->template_schedule_button, GX_STYLE_BUTTON_PUSHED);
            break;

        case ID_PATIENTS_SCREEN:
            gx_widget_style_add(&win->template_patients_button, GX_STYLE_BUTTON_PUSHED);
            break;
        }
        break;
    ...
    case GX_EVENT_HIDE:
        if (window->id == ID_VITALS_SCREEN)
        {
            gx_animation_delete(GX_NULL, (GX_WIDGET *)window);
        }
        else if (window->id == ID_SCHEDULE_SCREEN)
        {
