...
INT       y;
    ...
    get = canvas->memory;
    get += info->capture_ypos * canvas->x_resolution;
...
GX_PIXELMAP pixelmap;
    ...
    pixelmap.data = (GX_UBYTE *)info->capture_memory;
    pixelmap.data_size = info->capture_memory_size;
    pixelmap.width = CARDIO_DOT_WIDTH;
    pixelmap.height = CARDIO_DOT_HEIGHT;
    pixelmap.format = GX_COLOR_FORMAT_24XRGB;
...
INT           line_end_offset;
    ...
    xpos = chart_area->left + info->retrace_xpos + 10;
    gx_context_pixelmap_get(GX_PIXELMAP_ID_CHART_ERASE, &map);
    gx_context_fill_color_set(GX_COLOR_ID_CANVAS);
    gx_canvas_pixelmap_draw(xpos, chart_area->top, map);

    if (xpos + map->width > chart_area->right)
    {
        gx_canvas_pixelmap_draw(chart_area->left + xpos - chart_area->right,
                                chart_area->top, map);
    }

    xpos = chart_area->left +info->retrace_xpos;
    ...
    ycenter = (chart_area->top + chart_area->bottom) >> 1;
    ...
    if (chart_area->left + info->retrace_xpos + CHART_SCROLL > chart_area->right)
    {
        info->retrace_xpos = 0;
    }
}
...
static VOID start_value_animation()
{
    ...
    gx_slider_value_set((GX_SLIDER *)&vitals_screen.vitals_screen_medtype1_slider, &vitals_screen.vitals_screen_medtype1_slider.info, medtype1.start_value);
    gx_slider_value_set((GX_SLIDER*)&vitals_screen.vitals_screen_medtype2_slider, &vitals_screen.vitals_screen_medtype2_slider.info, medtype2.start_value);
    gx_slider_value_set((GX_SLIDER*)&vitals_screen.vitals_screen_medtype3_slider, &vitals_screen.vitals_screen_medtype3_slider.info, medtype3.start_value);
...
INT new_value;
    ...
    gx_slider_value_set((GX_SLIDER*)&vitals_screen.vitals_screen_medtype1_slider, &vitals_screen.vitals_screen_medtype1_slider.info, new_value);

    new_value = GET_NEW_VALUE(medtype2);
    gx_numeric_prompt_value_set(&vitals_screen.vitals_screen_medtype2_value, medtype2.start_value);
    gx_slider_value_set((GX_SLIDER*)&vitals_screen.vitals_screen_medtype2_slider, &vitals_screen.vitals_screen_medtype2_slider.info, new_value);

    new_value = GET_NEW_VALUE(medtype3);
    gx_numeric_prompt_value_set(&vitals_screen.vitals_screen_medtype3_value, medtype3.start_value);
    gx_slider_value_set((GX_SLIDER*)&vitals_screen.vitals_screen_medtype3_slider, &vitals_screen.vitals_screen_medtype3_slider.info, new_value);
...
UINT status = GX_SUCCESS;
    ...
    case GX_SIGNAL(ID_MEDTYPE1_SLIDER, GX_EVENT_SLIDER_VALUE):
        gx_numeric_prompt_value_set(&vitals_screen.vitals_screen_medtype1_value, myevent->payload.longdata);
        break;
        
    case GX_SIGNAL(ID_MEDTYPE2_SLIDER, GX_EVENT_SLIDER_VALUE):
        gx_numeric_prompt_value_set(&vitals_screen.vitals_screen_medtype2_value, myevent->payload.longdata);
        break;

    case GX_SIGNAL(ID_MEDTYPE3_SLIDER, GX_EVENT_SLIDER_VALUE):
        gx_numeric_prompt_value_set(&vitals_screen.vitals_screen_medtype3_value, myevent->payload.longdata);
        break;
    ...
    case GX_EVENT_ANIMATION_COMPLETE:
        if (myevent->sender == ID_EKG_WIN_SLIDE_IN)
        {
            start_ekg_waveform();
        }
        else if (myevent->sender = ID_PULSE_WIN_SLIDE_IN)
        {
...
VOID insulin_value_format(GX_NUMERIC_PROMPT *prompt, INT value)
{
    prompt->buffer[0] = '0';
    prompt->buffer[1] = '.';
    gx_utility_ltoa(value, prompt->buffer + 2, GX_NUMERIC_PROMPT_BUFFER_SIZE - 1);
}
