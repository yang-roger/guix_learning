...
UINT sequence_window_event_process(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    {
    case GX_EVENT_TIMER:
        if (event_ptr->payload.timer_id == SEQUENCE_TIMER)
        {
...
VOID task_icon_draw(GX_ICON *icon)
{
    ...
    if ((toggle_animation_flag == ANIMATION_NONE))
    {
        ...
        brush->alpha = task_title_blend_alpha;
    }
    ...
    if (current_task_icon_animation_step < TASK_ICON_ANIMATION_STEPS)
    {
        if (map)
        {
            ...
            /* Rotate task icon.  */
            gx_canvas_pixelmap_rotate(size->left, size->top, map, angle, -1, -1);
        }
    }
    else
    {
        /* Draw task icon.  */
        gx_canvas_pixelmap_draw(size->left, size->top, map);
    }
}
...
VOID task_title_animation_init()
{
    ...
    size = &sequence_window.sequence_window_task_title.size;

    if (size->left != TASK_TITLE_ANIMATION_START_X)
    {
        shift = TASK_TITLE_ANIMATION_START_X - size->left;
...
{
    gx_utility_ltoa(value, prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE);

    if (value < 10)
    {
        prompt->buffer[1] = prompt->buffer[0];
        prompt->buffer[0] = '0';
        prompt->buffer[2] = '\0';
    }
}
...
VOID mode_value_format(GX_NUMERIC_PROMPT *prompt, INT value)
{
    ...
    index = string_length_get(prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE - 1);
    prompt->buffer[index++] = '.';
    ...
    gx_utility_ltoa(value % 100, prompt->buffer + index, GX_NUMERIC_PROMPT_BUFFER_SIZE - index);
}
...
VOID position_value_format(GX_NUMERIC_PROMPT *prompt, INT value)
{
    ...
    index = string_length_get(prompt->buffer, GX_NUMERIC_PROMPT_BUFFER_SIZE);
    prompt->buffer[index++] = '.';
    gx_utility_ltoa(value % 10, prompt->buffer + index, GX_NUMERIC_PROMPT_BUFFER_SIZE - index);
}
