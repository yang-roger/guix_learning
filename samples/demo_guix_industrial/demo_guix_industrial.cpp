...
static VOID decode_main_screen_jpeg()
{
    ...
    gx_image_reader_create(&reader, map->data, map->data_size, GX_COLOR_FORMAT_24XRGB, 0);
...
VOID main_screen_draw(GX_WINDOW *window)
{
    ...
    {
        /* Draw background map. */
        gx_canvas_pixelmap_draw(window->size.left,
            window->size.top, &main_screen_bg);
    }
...
UINT main_screen_event_process(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_ON_OFF, GX_EVENT_CLICKED):
        if (main_screen.main_screen_prompt_on_off.text_id_() == GX_STRING_ID_TURN_ON)
        {
            /* Toggle to sequence screen.  */
            screen_toggle_animation_start((GX_WINDOW *)&sequence_window, current_screen);
        }
    ...
    case GX_SIGNAL(ID_PALLETIZING, GX_EVENT_CLICKED):
        if (current_screen->id == ID_SEQUENCE_WINDOW)
        {
            /* Pass event to sequence window.  */
            sequence_window.event_process_function((GX_WIDGET *)window, event_ptr);
        }
        else
        {
            GX_WIDGET *find;
            gx_widget_find(&main_screen, event_ptr->sender, GX_SEARCH_DEPTH_INFINITE, &find);
            if (find)
            {
...
VOID button_indicator_attach(GX_PIXELMAP_BUTTON *button)
{
    ...
    if ((GX_WIDGET *)button != indicator->parent)
    {
        gx_widget_attach((GX_WIDGET *)button, indicator);
    }

    x_shift = button->size.right - 18 - indicator->size.right;
    y_shift = button->size.top + 16 - indicator->size.top;
...
VOID screen_toggle_animation_start(GX_WINDOW *show, GX_WINDOW *hide)
{
    ...
    my_event.target = (GX_WIDGET *)show;
    ...
    case ID_MAIN_SCREEN:
        ...
        my_event.type = USER_EVENT_START_WIN_FADE_IN;
...
#else
        my_event.type = USER_EVENT_SEQ_WIN_FADE_IN;
        gx_system_event_send(&my_event);
#endif
        break;
    ...
    case ID_COMPLETE_WINDOW:
        ...
        my_event.type = USER_EVENT_COMPLETE_WIN_FADE_IN;
        gx_system_event_send(&my_event);
        break;
    ...
    my_event.target = (GX_WIDGET*)hide;
    ...
    case ID_MAIN_SCREEN:
        ...
        my_event.type = USER_EVENT_START_WIN_FADE_OUT;
        gx_system_event_send(&my_event);
        break;
    ...
    case ID_SEQUENCE_WINDOW:
        ...
        gx_animation_start(animation, &info);
#else
        my_event.type = USER_EVENT_SEQ_WIN_FADE_OUT;
        gx_system_event_send(&my_event);
#endif
        break;
    ...
    case ID_COMPLETE_WINDOW:
        ...
        my_event.type = USER_EVENT_COMPLETE_WIN_FADE_OUT;
        gx_system_event_send(&my_event);
        break;
...
VOID mode_button_draw(GX_PIXELMAP_BUTTON *button)
{
    ...
    {
        gx_widget_style_add(button->first_child, GX_STYLE_DRAW_SELECTED);
    }
    else
    {
        gx_widget_style_remove(button->first_child, GX_STYLE_DRAW_SELECTED);
    }
