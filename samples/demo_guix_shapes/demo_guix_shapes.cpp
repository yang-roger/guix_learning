...
GX_STRING string;
    ...
    if(pp)
    {
        ...
        string.ptr = str;
        string.length = string_length_get(str, sizeof(str) - 1);
...
UINT ShapesScreen_event_handler(GX_WINDOW *window, GX_EVENT *myevent)
{
    ...
    case GX_SIGNAL(ID_CIRCLE, GX_EVENT_RADIO_SELECT):
        if (pShapesWindow->parent == GX_NULL)
        {
...
INT      ycenter;
    ...
    xcenter = window -> size.left + ((window -> size.right - window -> size.left + 1) >> 1);
    ycenter = window -> size.top + ((window -> size.bottom - window -> size.top + 1) >> 1);
