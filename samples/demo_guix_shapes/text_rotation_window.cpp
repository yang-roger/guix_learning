...
void text_window_draw(GX_WINDOW *window)
{
    ...
    xpos = (window -> size.left + window -> size.right) / 2;
    ypos = (window -> size.top + window -> size.bottom) / 2;
    ...
    if (source_map)
    {
        if (text_angle)
        {
            if (gx_utility_pixelmap_rotate(source_map, text_angle, &rotated_text, &xpos, &ypos) == GX_SUCCESS)
            {
                xpos = (window -> size.left + window -> size.right) / 2;
                ypos = (window -> size.top + window -> size.bottom) / 2;
                xpos -= rotated_text.width >> 1;
                ypos -= rotated_text.height >> 1;
                gx_canvas_pixelmap_draw(xpos, ypos, &rotated_text);
                memory_free((VOID *)rotated_text.data);
            }
        }
        else
        {
            xpos -= source_map->width >> 1;
            ypos -= source_map->height >> 1;
    ...
    else
    {
        ...
        string.ptr = dynamic_text;
        string.length = string_length_get(dynamic_text, MAX_DYNAMIC_TEXT_LENGTH);
...
INT         *xshift;
INT         *yshift;
GX_RECTANGLE size = Text_Rotaion_Window.Text_Rotaion_Window_Text_Window.size;
    ...
    if (*xshift || *yshift)
    {
        ...
        if (*xshift > 0)
        {
            if (window->size.right >= size.right)
            {
                *xshift = -*xshift;
            }
        }
        ...
        if (*yshift > 0)
        {
            if (window->size.bottom >= size.bottom)
            {
                *yshift = -*yshift;
            }
        }
...
GX_PROMPT *prompt;
    ...
    string.ptr = str_value;
    string.length = string_length_get(str_value, sizeof(str_value) - 1);
